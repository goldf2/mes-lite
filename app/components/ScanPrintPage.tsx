...
import { pc310t203Profile } from './scan-print/labelProfiles'
...
function GenericLabel({ data }: { data: LabelData }) {
  ...
  return (
    <section className="shipment-label-sheet">
      <div className="border-b-2 border-black pb-3">
        <div className="text-[12px] font-semibold tracking-[0.18em]">{data.title || 'MES-lite 标签'}</div>
        <div className="mt-2 font-mono text-[19px] font-bold">{data.code || 'NO-CODE'}</div>
      </div>
      <div className="grid flex-1 grid-rows-[auto_auto_1fr_auto] gap-4 py-4">
        ...
        <div className="text-[13px] leading-relaxed">{data.note || 'PC310T 203 dpi · 100 × 150 mm'}</div>
...
export default function ScanPrintPage({ onMessage }: { onMessage: (message: string) => void }) {
  ...
  const requestPrint = async () => {
    ...
    try {
      ...
      const cleanup = () => document.body.classList.remove('printing-shipment-label')
  ...
  return (
    <div className="space-y-4">
        ...
        <section className="space-y-4 rounded-lg border border-gray-200 bg-white p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">203 dpi · 网线连接 · 100×150 mm · 不绑定扫码会话</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 rounded-lg bg-gray-50 p-3 text-xs text-gray-600 sm:grid-cols-4">
            ...
            <div><div className="text-gray-400">标签</div><div className="mt-1 font-medium">100×150 mm</div></div>
            <div><div className="text-gray-400">画布</div><div className="mt-1 font-medium">800×1200 dots</div></div>
          </div>
          <div className="rounded-lg bg-amber-50 p-3 text-xs leading-relaxed text-amber-900">首次使用请在 PC310T 驱动中创建 100×150 mm 纸张并校准标签间隙。浏览器打印选择无边距、100% 缩放。打印机 IP 当前仅保存配置，网络静默打印后续由独立桥接模块接入。</div>
      ...
      <div className="shipment-label-print-root" aria-hidden="true">
        {Array.from({ length: copies }, (_, index) => <GenericLabel key={index} data={labelData} />)}
      </div>
    </div>
  )
}
