...
export default function BomRelationPage({ onMessage }: { onMessage: (msg: string) => void }) {
  const [products, setProducts] = useState<ProductBom[]>([])
  ...
  const loadData = useCallback(async (nextSelectedProductId: string) => {
    ...
    try {
      ...
      const preferred = nextProducts.find((product: ProductBom) => product.id === nextSelectedProductId)
      const withBom = nextProducts.find((product: ProductBom) => product.bom?.items.some((item) => item.itemType === 'MATERIAL'))
  ...
  const usageRows = useMemo(() => {
    const rows: Array<{ material: MaterialOption; products: Array<{ product: ProductBom; item: BomItem }> }> = []
      ...
      <div className="rounded-lg bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <div className="mt-1 text-sm text-gray-500">成品和原材料用量关系 · 后续可继续接入成本计算</div>
          </div>
          <button
            type="button"
            onClick={saveBom}
        ...
        <div className="space-y-4">
          <div className="rounded-lg bg-white p-5 shadow-sm">
            <div className="mb-3 flex items-center justify-between gap-3">
              <h3 className="font-semibold text-gray-900">选择成品</h3>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-lg border border-blue-100 bg-blue-50 p-3">
              <div className="text-xs text-blue-700">产品数</div>
        ...
        <div className="space-y-4">
          <div className="rounded-lg bg-white p-5 shadow-sm">
            <div className="mb-4 flex flex-col gap-1 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <h3 className="font-semibold text-gray-900">成品 BOM 明细</h3>
                <div className="mt-1 text-sm text-gray-500">这里只维护直接用料、用量、单位和损耗率</div>
              </div>
              <div className="text-xs text-gray-500">{selectedProduct?.bom ? selectedProduct.bom.version : '未创建 BOM'}</div>
            </div>
            {draftItems.length === 0 ? (
            ...
            {advancedBomItems.length > 0 && (
              <div className="mt-4 rounded-lg border border-amber-100 bg-amber-50 p-3 text-sm text-amber-800">
                当前产品还包含 {advancedBomItems.length} 个成本对象或锯切成本项，本页保存物料 BOM 时会保留这些高级项。
              </div>
            )}
          </div>
          ...
          <div className="rounded-lg bg-white p-5 shadow-sm">
            <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <div className="mt-1 text-sm text-gray-500">查看某个物料被哪些成品 BOM 使用</div>
              </div>
              <input
                value={keyword}
                ...
                {usageRows.map((row) => (
                  <div key={row.material.id} className="rounded-lg border border-gray-200 p-4">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="shrink-0 rounded bg-gray-100 px-2 py-1 text-xs text-gray-600">{row.products.length} 个成品</span>
                    </div>
                    <div className="mt-3 space-y-2">
                      {row.products.slice(0, 6).map(({ product, item }) => (
                        <button
                          ...
                          className="flex w-full items-center justify-between gap-3 rounded-lg bg-gray-50 px-3 py-2 text-left text-xs text-gray-700 hover:bg-blue-50 hover:text-blue-700"
                        >
                          <span className="min-w-0 truncate">{product.sku} · {product.name}</span>
                          <span className="shrink-0">{qty(item.quantity, 4)} {item.unit}</span>
                        </button>
                      ))}
                      {row.products.length > 6 && <div className="px-3 text-xs text-gray-500">还有 {row.products.length - 6} 个成品</div>}
                    </div>
                  </div>
                ))}
              </div>
