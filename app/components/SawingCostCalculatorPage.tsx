  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        {(['DIRECT', 'LABOR', 'FIXED'] as CostStage[]).map((stage) => <CostStageEditor key={stage} stage={stage} items={costItems.filter((item) => item.stage === stage)} onChange={(items) => setCostItems((current) => [...current.filter((item) => item.stage !== stage), ...items])} />)}
      </div>
        ...
        <div className="space-y-4">
          {result.quantity === 0 ? (
            ...
            <>
              ...
              <div className="rounded-lg bg-white p-5 shadow-sm">
                <h3 className="font-semibold text-gray-900">保存计算方案</h3>
                <p className="mt-1 text-xs text-gray-500">可将锯切与钻孔、车削、铣削等工艺组合记录在同一方案中。</p>
                <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
                  <input value={scenarioName} onChange={(event) => setScenarioName(event.target.value)} placeholder="方案名称，如：50 圆钢薄锯片方案" className="rounded-lg border border-gray-200 px-3 py-2 text-sm" />
                  <button onClick={saveScenario} disabled={saving} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{saving ? '保存中...' : '保存当前方案'}</button>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  {processOptions.map((process) => <label key={process.id} className={`cursor-pointer rounded-full border px-3 py-1.5 text-xs ${selectedProcessIds.includes(process.id) ? 'border-blue-300 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600'}`}><input type="checkbox" className="mr-1.5" checked={selectedProcessIds.includes(process.id)} onChange={(event) => setSelectedProcessIds(event.target.checked ? [...selectedProcessIds, process.id] : selectedProcessIds.filter((id) => id !== process.id))} />{process.name}</label>)}
                </div>
              </div>
            </>
          )}
