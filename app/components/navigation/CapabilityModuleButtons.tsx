...
export default function CapabilityModuleButtons({
  ...
  return (
    <div
      role="group"
      aria-label="产品模块"
      className={`flex min-w-0 flex-1 rounded-lg border border-gray-200 bg-gray-100 p-1 ${compact ? 'h-9' : 'h-10'}`}
    >
      {visibleModules.map((moduleId) => {
        ...
        return (
          <button
            ...
            className={`relative min-w-0 rounded-md px-1.5 text-xs font-bold tracking-wide transition ${
              active
                ? 'flex-[1.45] bg-blue-600 text-white shadow-sm'
