...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const denied = await requireResourcePermission('flowTransferConfirm', 'update')
    if (denied) return denied
    confirmFlowTransferSchema.parse(await req.json().catch(() => ({})))
    const operator = await getCurrentOperator()
    if (!operator) return NextResponse.json({ error: '请先登录' }, { status: 401 })
    const { current, updated } = await confirmManagedFlowTransfer(params.id, operatorDisplayName(operator), new Date(), await loadEffectiveDataScope(operator))
