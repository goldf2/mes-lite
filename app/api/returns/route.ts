...
import { returnSearchFieldKeys } from '@/modules/sales/model/sales-search-fields'
...
export async function GET(req: NextRequest) {
  try {
    ...
    const advanced = parseResourceSearchConditions(params.get('advanced'), returnSearchFieldKeys)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const operator = await getCurrentOperator()
    if (!operator) return NextResponse.json({ error: '无权限' }, { status: 403 })
    const returnOrder = await createManagedReturn(createReturnSchema.parse(await req.json()), new Date(), await loadEffectiveDataScope(operator))
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const { before, updated } = await archiveManagedReturn(id, await loadEffectiveDataScope(operator))
    await writeAuditLog(req, {
