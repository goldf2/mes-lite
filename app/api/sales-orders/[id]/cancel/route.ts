...
import { cancelManagedSalesOrder } from '@/modules/sales/server/sales-order-command-service'

export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const denied = await requireResourcePermission('salesOrder', 'update')
    if (denied) return denied
    const { before, updated } = await cancelManagedSalesOrder(params.id)
    await writeAuditLog(req, {
      action: 'CANCEL', entityType: 'SALES_ORDER', entityId: before.id,
