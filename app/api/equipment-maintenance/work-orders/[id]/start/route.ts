...
import { startEquipmentMaintenanceWorkOrder } from '@/modules/equipment/server/equipment-maintenance-command-service'
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await startEquipmentMaintenanceWorkOrder(params.id, {
  ...
  } catch (error) { return equipmentMaintenanceHttpError(error, '开始设备维修工单失败') }
}
