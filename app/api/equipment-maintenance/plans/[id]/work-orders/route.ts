...
import { createPreventiveMaintenanceWorkOrderSchema } from '@/modules/equipment/contracts/equipment-maintenance-schema'
import { equipmentMaintenanceHttpError } from '@/modules/equipment/http/equipment-maintenance-http'
import { createPreventiveMaintenanceWorkOrder } from '@/modules/equipment/server/equipment-maintenance-command-service'
...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const result = await createPreventiveMaintenanceWorkOrder(params.id, createPreventiveMaintenanceWorkOrderSchema.parse(await req.json()), {
  ...
  } catch (error) { return equipmentMaintenanceHttpError(error, '生成设备保养工单失败') }
}
