...
import { BusinessDocumentPrintData, renderBusinessDocumentPdf } from '@/lib/business-document-pdf'
import { attachmentUploadRoot, resolveAttachmentStoragePath } from '@/lib/attachment-storage'

export const runtime = 'nodejs'
export const dynamic = 'force-dynamic'

const permissionByKind: Record<string, PermissionResource> = {
  'material-in': 'materialIn',
...
const ownerTypeByKind: Record<string, string> = {
  'material-in': 'MATERIAL_IN',
  'sales-order': 'SALES_ORDER',
  shipment: 'SHIPMENT',
  return: 'RETURN_ORDER',
  'flow-transfer': 'FLOW_TRANSFER',
  'production-order': 'PRODUCTION_ORDER',
  dispatch: 'DISPATCH',
}

const GENERATED_PDF_TYPE = 'SYSTEM_GENERATED_PDF'
...
const priorityLabels: Record<string, string> = { LOW: '低', NORMAL: '正常', HIGH: '高', URGENT: '紧急' }
const dateText = (value?: Date | string | null) => value ? new Date(value).toLocaleString('zh-CN', { hour12: false }) : '-'
const numberText = (value: number) => Number(value || 0).toFixed(3).replace(/\.?0+$/, '')
const money = (value: number) => `¥${Number(value || 0).toFixed(2)}`

async function loadPrintData(kind: string, id: string): Promise<BusinessDocumentPrintData | null> {
  if (kind === 'sales-order') {
...
export async function GET(
  ...
  const resource = permissionByKind[params.kind]
  if (!resource) return NextResponse.json({ error: '不支持的单据类型' }, { status: 404 })

  try {
    const denied = await requireResourcePermission(resource, 'read')
    if (denied) return denied
    const ownerType = ownerTypeByKind[params.kind]
    const regenerate = new URL(request.url).searchParams.get('regenerate') === '1'
    const archivedPdf = await prisma.documentAttachment.findFirst({
      where: { ownerType, ownerId: params.id, documentType: GENERATED_PDF_TYPE, deletedAt: null },
      orderBy: { createdAt: 'desc' },
    })
    if (regenerate) {
      const generationDenied = await requireResourcePermission(resource, archivedPdf ? 'update' : 'create')
      if (generationDenied) return generationDenied
    }
    if (archivedPdf && !regenerate) {
      const pdf = await readFile(resolveAttachmentStoragePath(archivedPdf.storagePath))
      return pdfResponse(pdf, archivedPdf.originalName)
    }

    const [data, settings] = await Promise.all([loadPrintData(params.kind, params.id), getSystemSettings()])
    if (!data) return NextResponse.json({ error: '单据不存在或已归档' }, { status: 404 })

    const pdf = await renderBusinessDocumentPdf(data, settings)
    const originalName = `${data.title}-${data.documentNo}.pdf`
    const ownerDirectory = path.join(attachmentUploadRoot(), ownerType, params.id)
    ...
    await prisma.documentAttachment.create({
      data: {
        ownerType,
        ownerId: params.id,
        documentType: GENERATED_PDF_TYPE,
        originalName,
        fileName,
        mimeType: 'application/pdf',
        size: pdf.byteLength,
        url: `/api/business-documents/${params.kind}/${params.id}/print`,
        storagePath,
        note: regenerate && archivedPdf ? '单据保存后重新生成的归档版本' : '单据创建后自动生成的归档版本',
      },
    })
    return pdfResponse(pdf, originalName)
  } catch (error) {
