...
The service converts DWG to DXF with GNU LibreDWG and renders DXF layouts to a read-only PDF with ezdxf/PyMuPDF. Original attachments remain in MES-lite; this service only uses an isolated temporary directory and does not retain uploads.

Build from the repository root:

```bash
docker build -f services/cad-preview/Dockerfile -t mes-lite-cad-preview:0.1.416 .
```
