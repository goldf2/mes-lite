...
assert.equal(normalizeScanCode(' mat-P12-001\r\n'), 'MAT-P12-001', '扫码规范化必须保留真实编码的 MAT- 前缀')
assert.deepEqual(classifyScan({ rawValue: 'P12-001', expectedCode: 'MAT-P12-001', countedQty: 0, expectedQty: 2, quantity: 1 }), { code: 'P12-001', result: 'UNKNOWN' })
assert.deepEqual(classifyScan({ rawValue: 'MAT-P12-001', expectedCode: 'P12-001', countedQty: 0, expectedQty: 2, quantity: 1 }), { code: 'MAT-P12-001', result: 'UNKNOWN' })
assert.deepEqual(classifyScan({ rawValue: ' mat-P12-001\r\n', expectedCode: 'MAT-P12-001', countedQty: 0, expectedQty: 2, quantity: 1 }), { code: 'MAT-P12-001', result: 'MATCHED' })
...
async function main() {
  ...
  try {
    ...
    assert.equal(session.expectedCode, 'MAT-VERIFY-CODE')
    assert.equal((await createScanSession(sessionInput, '重复请求')).id, session.id, '会话创建必须幂等')
    const differentCode = await recordScanEvent(session.id, recordScanEventSchema.parse({ clientEventId: `EVENT-UNKNOWN-${suffix}`, rawValue: 'VERIFY-CODE', quantity: 1 }))
    assert.equal(differentCode.scanResult, 'UNKNOWN', '有无 MAT- 前缀的真实编码不得自动视为同一物料')
    assert.equal(differentCode.data.countedQty, 0, '不同物料编码不得增加扫码数量')
    await recordScanEvent(session.id, recordScanEventSchema.parse({ clientEventId: `EVENT-1-${suffix}`, rawValue: 'MAT-VERIFY-CODE', quantity: 1 }), auditContext)
    const second = await recordScanEvent(session.id, recordScanEventSchema.parse({ clientEventId: `EVENT-2-${suffix}`, rawValue: 'MAT-VERIFY-CODE', quantity: 1 }), auditContext)
    assert.equal(second.data.countedQty, 2)
    assert.equal((await recordScanEvent(session.id, recordScanEventSchema.parse({ clientEventId: `EVENT-2-${suffix}`, rawValue: 'MAT-VERIFY-CODE', quantity: 1 }), auditContext)).data.countedQty, 2, '扫码事件必须幂等')
    ...
    await recordScanEvent(session.id, recordScanEventSchema.parse({ clientEventId: `EVENT-3-${suffix}`, rawValue: 'MAT-VERIFY-CODE', quantity: 1 }), auditContext)
