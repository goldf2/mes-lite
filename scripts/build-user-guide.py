...
def add_front_matter(doc: Document, bullet_num_id: int) -> None:
    ...
    bullets = [
        ...
        "供应商批号、来料内部批次、生产投入、产出批次、客户发货、退货回流、质量判定与不合格处置已贯通；48 项细粒度资源、岗位任务、生产命令、数据范围和临时授权已落地；生产已建立固定提交、每日一致备份和失败通知，生产恢复候选已完成隔离应用启动、登录、业务与附件抽查；Product 到 Material 仍待人工确认和回填，异地副本、真实 Coolify 挂载切换与真实岗位审批仍待完成。",
