...
def add_cover(doc: Document) -> None:
    ...
    rows = [
        ("交付版本", f"v{VERSION}"),
        ("截图基线", "v0.1.350-v0.1.358 的 112 张真实页面流程截图"),
...
def markdown_lines() -> Iterable[str]:
    ...
    yield "- 截图基线：v0.1.350-v0.1.358（112 张真实页面流程截图）"
    yield "- 数据范围：隔离本地临时演示库，不含生产业务数据"
    yield "- 编制日期：2026-08-13"
    yield ""
    yield "> 重要：供应商批号、来料内部批次、生产投入产出、客户发货、退货回流、质量判定、不合格处置和跨批次搜索全景已贯通；本机临时数据与截图不等于真实 Coolify 服务器已部署。"
    yield ""
    yield "## 推荐业务顺序"
    yield ""
