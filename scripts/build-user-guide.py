...
def add_front_matter(doc: Document, bullet_num_id: int) -> None:
    ...
    bullets = [
        ...
        "供应商批号、来料内部批次、生产投入、产出批次、客户发货、退货回流、质量判定与不合格处置已贯通；61 项细粒度资源、岗位任务、生产/来料/物流命令、设备事件、周期点检/维保、生产实绩设备/文件版本快照、数据范围和临时授权已落地。2026-08-17 已只读核验真实 Coolify 的 v0.1.396 部署、三个显式持久挂载、live/ready、84 个迁移和最新一致备份，基础运行通过；Product 到 Material 人工确认和回填、真实岗位签字、关键业务闭环与回切演练、OSS/异地存储及自动设备采集/OEE 仍待人工跟进。",
