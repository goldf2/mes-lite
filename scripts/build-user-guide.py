...
def add_cover(doc: Document) -> None:
    ...
    rows = [
        ("交付版本", f"v{VERSION}"),
        ("截图基线", "v0.1.350-v0.1.357 的 106 张真实页面流程截图"),
...
def add_front_matter(doc: Document, bullet_num_id: int) -> None:
    ...
    bullets = [
        ...
        "供应商批号、来料内部批次、生产投入、产出批次、客户发货、退货回流、质量判定与不合格处置已贯通；生产上线前仍须在服务器维护窗口完成备份、迁移、岗位权限和真实业务验收。",
...
def markdown_lines() -> Iterable[str]:
    ...
    yield "- 截图基线：v0.1.350-v0.1.357（106 张真实页面流程截图）"
    yield "- 数据范围：隔离本地临时演示库，不含生产业务数据"
    yield "- 编制日期：2026-08-13"
    yield ""
    yield "> 重要：供应商批号、来料内部批次、生产投入产出、客户发货、退货回流、质量判定与不合格处置已贯通；本机临时数据与截图不等于真实 Coolify 服务器已部署。"
    yield ""
    yield "## 推荐业务顺序"
    yield ""
    ...
    yield ""
    yield "- 质量闭环已贯通；正式上线前仍须按企业审批制度配置岗位权限并使用真实不合格评审单验收。"
    yield "- 供应商、来料、生产投入产出、发货和退货回流已形成谱系；跨工厂或供应商系统集成不在本阶段。"
