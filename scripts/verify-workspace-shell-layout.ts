...
assert.doesNotMatch(shellSource, /scheduleDesktopNavigationOpen|scheduleDesktopNavigationClose|openTransientDesktopNavigation/, '自动隐藏左栏不得因鼠标经过边缘、按钮或面板而自行开合')
assert.match(shellSource, /bg-white shadow-lg transition-transform/, '常驻和自动隐藏左栏必须复用同一面板阴影外观')
assert.doesNotMatch(desktopNavigationControllerSource, /openTimerRef|closeTimerRef|scheduleOpen|scheduleClose/, '桌面导航控制器不得保留竞争性的悬停开合计时器')
