...
const canvasSource = readFileSync(join(root, 'modules/inventory/ui/WarehouseTwinCanvas.tsx'), 'utf8')
const detailSource = readFileSync(join(root, 'modules/inventory/ui/WarehouseTwinDetailPanel.tsx'), 'utf8')
const quickTransferSource = readFileSync(join(root, 'modules/production/ui/FlowTransferQuickDialog.tsx'), 'utf8')
const transferEntrySource = readFileSync(join(root, 'modules/production/ui/FlowTransferEntryDialog.tsx'), 'utf8')
...
assert.match(pageSource, /initialFullscreen/, '仓库白板必须能进入视口全屏模式')
assert.match(pageSource, /FlowTransferQuickDialog/, '仓库白板必须复用流程转移快捷入口')
assert.match(canvasSource, /min-h-\[720px\]/, '普通页面的仓库白板必须提供足够的纵向显示区域')
assert.match(canvasSource, /全屏显示仓库白板/, '仓库白板工具栏必须提供明确的全屏入口')
assert.match(detailSource, /material\.availableQty > 0\.000001/, '只有来源库位可用库存才能发起转移')
assert.match(quickTransferSource, /saveFlowTransfer\(form\)/, '白板快捷转移必须复用既有转移单创建接口')
assert.match(quickTransferSource, /confirmFlowTransfer\(created\.transfer\.id\)/, '有确认权限时白板快捷转移必须复用既有确认接口')
assert.match(quickTransferSource, /转移草稿 .* 已保存，但确认失败/, '创建成功但确认失败时必须明确保留草稿')
assert.match(transferEntrySource, /sourceLocationLocked/, '白板快捷转移必须锁定已选来源库位')
assert.match(registrySource, /key: 'warehouseDigitalTwin'/)
assert.match(rendererSource, /'warehouse-digital-twin'/)
assert.match(rendererSource, /canCreateFlowTransfer=\{context\.canRead\('flowTransfers'\) && context\.canCreate\('flowTransfers'\)\}/)
assert.match(rendererSource, /canConfirmFlowTransfer=\{context\.canUpdate\('flowTransferConfirm'\)\}/)
...
async function verifyIntegrityWarningProjection() {
  ...
  try {
    ...
    console.log('仓库数字孪生 MVP 验证通过：独立只读查询、完整性警告、扩展画布、全屏查看及复用权限化流程转移入口。')
  } finally {
