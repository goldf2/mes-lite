...
assert.match(attachmentPreviewRouteSource, /stat\(previewPath\)[\s\S]*mtimeMs/, 'CAD 全屏预览 ETag 必须随派生 PDF 更新')
assert.match(attachmentPreviewRouteSource, /kind === 'cad'[\s\S]*private, no-cache/, 'CAD 全屏预览必须在再次打开时重新验证缓存')
