...
const validReasons = validateStockBalance({
  qty: 10, reservedQty: 3, availableQty: 5, quarantineQty: 1, holdQty: 1, reworkQty: 0,
  valuationQty: 20, reservedValuationQty: 4, availableValuationQty: 12, quarantineValuationQty: 2, holdValuationQty: 2, reworkValuationQty: 0,
  totalCost: 100, quarantineCost: 10, holdCost: 10, reworkCost: 0, hasMaterial: true, hasProduct: false,
  materialExists: true, productExists: false,
  locationBalances: [{ qty: 10, reservedQty: 3, availableQty: 5, quarantineQty: 1, holdQty: 1, reworkQty: 0 }],
})
assert.deepEqual(validReasons, [], '平衡库存不得产生一致性错误')
const invalidReasons = validateStockBalance({
  qty: 5, reservedQty: 6, availableQty: 1, quarantineQty: 0, holdQty: 0, reworkQty: 0,
  valuationQty: 5, reservedValuationQty: 0, availableValuationQty: 5, quarantineValuationQty: 0, holdValuationQty: 0, reworkValuationQty: 0,
  totalCost: 0, quarantineCost: 0, holdCost: 0, reworkCost: 0, hasMaterial: true, hasProduct: true,
  materialExists: true, productExists: true,
  locationBalances: [{ qty: 4, reservedQty: 0, availableQty: 4, quarantineQty: 0, holdQty: 0, reworkQty: 0 }],
})
