...
assert.match(page, /from '@\/modules\/materials'/, '销售订单页必须通过 materials 公开出口复用物料详情能力')
assert.equal((page.match(/<MaterialReferenceButton/g) || []).length, 2, '销售订单卡片和详情中的物料必须统一提供详情入口')
assert.match(page, /<MaterialDetailDialog[\s\S]*material=\{detailMaterial\}/, '销售订单页必须复用物料领域详情弹窗')
assert.match(page, /\{detailOrder && !detailMaterial && \(/, '订单内查看物料时必须在同一弹窗层级切换，避免叠加多层对话框')
