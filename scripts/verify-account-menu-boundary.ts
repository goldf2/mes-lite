...
const navigationSource = readFileSync(resolve(process.cwd(), 'app/components/shell/useApplicationNavigationController.tsx'), 'utf8')
...
assert.match(navigationSource, /id:\s*'account'[\s\S]*?items:\s*readableSystemNavItems\.map/, '账号与权限管理必须保留在一级导航中')
