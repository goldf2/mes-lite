...
assert.doesNotMatch(materialPage, /saveMaterial|importMaterials|createEmptyMaterialForm/, '主物料页面不得重新承载保存、导入或表单初始化状态')
...
assert.ok(materialPage.split('\n').length <= 2600, '物料页面已拆出编辑与导入切片，不得再超过 2600 行')
...
console.log('物料/BOM 模块边界验证通过：页面归属、公开出口、数据契约、HTTP client、编辑与导入切片已分离。')
