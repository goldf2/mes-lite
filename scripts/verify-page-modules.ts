...
const expectedFunctionKeys = [
  ...
  'equipment', 'equipmentInspections', 'equipmentMaintenance', 'orders', 'flowTransfers', 'dispatch', 'qualityTasks', 'materialIn', 'salesOrders', 'shipment', 'return',
...
const materialCardViewSource = readFileSync(join(root, 'modules/materials/ui/MaterialCardView.tsx'), 'utf8')
const workInstructionPageSource = readFileSync(join(root, 'modules/documents/ui/WorkInstructionPage.tsx'), 'utf8')
const workInstructionCollectionSource = readFileSync(join(root, 'modules/documents/ui/WorkInstructionCollectionView.tsx'), 'utf8')
const salesOrderPageSource = readFileSync(join(root, 'modules/sales/ui/SalesOrderPageModule.tsx'), 'utf8')
const flowTransferPageSource = readFileSync(join(root, 'modules/production/ui/FlowTransferPageModule.tsx'), 'utf8')
...
assert.doesNotMatch(materialCardViewSource, /repeat\(auto-fit/, '物料卡片不得在单条结果时拉伸占满整行')
assert.doesNotMatch(workInstructionCollectionSource, /repeat\(auto-fit/, '文档卡片不得在单条结果时拉伸占满整行')
assert.match(materialCardViewSource, /md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4/, '物料卡片必须使用最多四列的明确响应式网格')
assert.match(workInstructionPageSource, /<WorkInstructionCollectionView/, '文档主页必须装配公共集合视图')
assert.match(workInstructionCollectionSource, /md:grid-cols-2 2xl:grid-cols-3/, '文档卡片必须使用最多三列的明确响应式网格')
...
assert.match(pageAuditSource, /固定形态合理：21 个/, '页面标准化审计的固定形态统计必须保持同步')
