...
assert.equal((shellSource.match(/<CapabilityModuleButtons/g) || []).length, 2, '桌面品牌区和移动菜单必须显示共享模块按钮')
...
assert.doesNotMatch(moduleButtonsSource, /onChange|onClick/, '模块按钮不得携带工作区切换动作')
...
console.log('统一 MES 工作台、可配置预留模块按钮、历史三工作区配置合并、菜单顺序与别名验证通过')
