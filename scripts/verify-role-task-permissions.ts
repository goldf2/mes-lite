...
async function main() {
  ...
  try {
    ...
    assert.equal(await hasResourcePermission(warehouse, 'shipmentDispatch', 'update'), true, '仓管可执行库存发出')
    assert.equal(await hasResourcePermission(warehouse, 'shipmentDeliver', 'update'), false, '仓管不得代替销售确认客户签收')
    assert.equal(await hasResourcePermission(warehouse, 'shipmentCancel', 'update'), false, '仓管不得取消发货单')
    assert.equal(await hasResourcePermission(warehouse, 'shipmentReverse', 'update'), false, '仓管不得冲销已发货单')
    assert.equal(await hasResourcePermission(warehouse, 'returnReceive', 'update'), true, '仓管可接收入库退货')
    assert.equal(await hasResourcePermission(warehouse, 'returnReject', 'update'), false, '仓管不得拒绝退货申请')
    assert.equal(await hasResourcePermission(warehouse, 'flowTransferConfirm', 'update'), true, '仓管可确认移库')
    assert.equal(await hasResourcePermission(warehouse, 'flowTransferReverse', 'update'), false, '仓管不得冲销移库')
    assert.equal(await hasResourcePermission(warehouseLead, 'shipmentCancel', 'update'), true, '仓库主管可取消待发货单')
    assert.equal(await hasResourcePermission(warehouseLead, 'shipmentReverse', 'update'), true, '仓库主管可执行有原因发货冲销')
    assert.equal(await hasResourcePermission(warehouseLead, 'returnReject', 'update'), true, '仓库主管可拒绝不符合条件的退货')
    assert.equal(await hasResourcePermission(warehouseLead, 'flowTransferReverse', 'update'), true, '仓库主管可执行有原因移库冲销')
    ...
    assert.match(read('app/api/shipments/[id]/ship/route.ts'), /requireResourcePermission\('shipmentDispatch', 'update'\)/, '发货执行 API 必须使用独立权限')
    assert.match(read('app/api/shipments/[id]/deliver/route.ts'), /requireResourcePermission\('shipmentDeliver', 'update'\)/, '签收 API 必须使用独立权限')
    assert.match(read('app/api/shipments/[id]/cancel/route.ts'), /requireResourcePermission\('shipmentCancel', 'update'\)/, '发货取消 API 必须使用独立权限')
    assert.match(read('app/api/shipments/[id]/reverse/route.ts'), /requireResourcePermission\('shipmentReverse', 'update'\)/, '发货冲销 API 必须使用独立权限')
    assert.match(read('app/api/returns/[id]/process/route.ts'), /requireResourcePermission\('returnReceive', 'update'\)/, '退货收货 API 必须使用独立权限')
    assert.match(read('app/api/returns/[id]/reject/route.ts'), /requireResourcePermission\('returnReject', 'update'\)/, '退货拒绝 API 必须使用独立权限')
    assert.match(read('app/api/flow-transfers/[id]/confirm/route.ts'), /requireResourcePermission\('flowTransferConfirm', 'update'\)/, '移库确认 API 必须使用独立权限')
    assert.match(read('app/api/flow-transfers/[id]/reverse/route.ts'), /requireResourcePermission\('flowTransferReverse', 'update'\)/, '移库冲销 API 必须使用独立权限')
