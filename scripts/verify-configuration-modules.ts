...
const documentsModuleIndex = read('modules/documents/index.ts')
...
assert.match(workspacePageRenderers, /import\('@\/modules\/documents'\).*DocumentCategorySettingsPage/, '文档类别必须通过文档领域公开入口加载')
for (const path of [
...
]) assert.ok(!existsSync(join(root, path)), `旧文档类别实现不得保留：${path}`)
assert.doesNotMatch(moduleIndex, /DocumentCategorySettingsPage/, '配置模块不得导出文档领域页面')
assert.match(documentsModuleIndex, /DocumentCategorySettingsPage/, '文档领域必须公开文档类别页面')
...
assert.match(referenceDataClient, /loadInventoryLocations|loadConfiguredUnits/, '配置领域 client 必须集中自身资料读取')
assert.doesNotMatch(referenceDataClient, /DocumentCategor|document-categories/, '配置领域 client 不得承载文档类别请求')
assert.doesNotMatch(referenceDataClient, /loadWorkCenters|saveWorkCenter|archiveWorkCenter/, '工作中心请求必须归属设备领域')
assert.match(referenceDataContracts, /interface PartyRecord|interface ConfiguredUnit|interface InventoryLocationConfig/, '配置领域必须集中自身参考资料契约')
assert.doesNotMatch(referenceDataContracts, /DocumentCategoryConfig/, '配置领域不得复制文档类别契约')
...
const categoryPage = read('modules/documents/ui/DocumentCategorySettingsPage.tsx')
...
console.log(`配置模块校验通过：${resourcePages.length} 个自有 ResourcePage，文档类别和工作中心分别经文档、设备领域公开入口挂载，SystemPage ${systemPage.split('\n').length} 行。`)
