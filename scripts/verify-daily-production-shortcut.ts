...
async function main() {
  ...
  try {
    ...
    assert.equal(report.actualMaterialCostAmount, 100, '生产日报应记录实际投入材料成本')
    assert.equal(report.actualProcessCostAmount, 10, '生产日报应按实际主产出数量缩放冻结加工成本')
    assert.equal(report.outputCostAmount, 110, '生产日报主产出库存成本应包含材料和加工成本')
    assert.equal(JSON.parse(report.appliedCostSnapshot || '{}').sourceRunId, oldCostRun.id, '生产日报应保存实际应用的冻结成本来源')
    const reportPrimaryOutput = report.outputs.find((line) => line.isPrimary)
    assert.equal(reportPrimaryOutput?.materialCostAmount, 100, '生产日报主产出应拆分记录实际材料成本')
    assert.equal(reportPrimaryOutput?.processCostAmount, 10, '生产日报主产出应拆分记录实际加工成本')
