...
function verifyBoundaries() {
  ...
  assert.match(detailDialog, /editing[\s\S]*\? 'grid-cols-1'/, '详情编辑模式必须让基础信息使用整行，不能继续塞入固定窄侧栏')
  ...
  assert.match(formFields, /detailMode[\s\S]*md:grid-cols-2 xl:grid-cols-4/, '详情编辑表单必须按可用宽度重排，标签始终位于完整宽度控件上方')
  assert.match(formFields, /function MaterialSearchSelect[\s\S]*<SearchableSelect[\s\S]*allowClear/, '文档关联产品必须复用公共可搜索选择器和可操作箭头')
  ...
  assert.match(collection, /min-w-\[1280px\] table-fixed[\s\S]*className="w-80">文档标题/, '文档表格必须保留稳定标题列宽，不能把日期和中文压成逐字换行')
