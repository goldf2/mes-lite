...
export default function InventoryLocationSettingsPage({ onMessage, canCreate, canUpdate, canDelete }: { onMessage: (message: string) => void; canCreate: boolean; canUpdate: boolean; canDelete: boolean }) {
  ...
  const actions = (item: InventoryLocationConfig) => <div className="flex justify-end gap-2">{canUpdate && <AppButton size="sm" onClick={() => openEdit(item)}>编辑</AppButton>}{canUpdate && item.isActive && !item.isDefault && <AppButton size="sm" variant="primary" onClick={() => void makeDefault(item)}>设为默认</AppButton>}{canDelete && item.isActive && !item.isDefault && <AppButton size="sm" variant="warning" onClick={() => void archive(item)}>归档</AppButton>}</div>
  ...
  return (
    <>
      <ResourcePage resourceKey="inventory-locations" title="库位配置" description="总库存继续统一核算；库位用于来料、生产订单实绩和发货的实物数量分布与校验。" items={tableSort.sortedRows} getKey={(item) => item.id} columns={columns}
        ...
        actions={canUpdate ? <ConfigurationManualOrder entity="locations" label="库位" onMessage={onMessage} onSaved={load} /> : undefined} viewMode={viewMode} onViewModeChange={setViewMode} displayModes={['card', 'list']} onCreate={canCreate ? openCreate : undefined} createLabel="新建库位" summary={<span className="text-sm text-gray-500">共 {filteredItems.length} 项</span>} rowLabel={(item) => `库位 ${item.code} ${item.name}`} />
