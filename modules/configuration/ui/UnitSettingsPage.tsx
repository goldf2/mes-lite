...
export default function UnitSettingsPage({ onMessage }: { onMessage: (message: string) => void }) {
  ...
  const actions = (item: ConfiguredUnit) => item.isPreset ? <span className="text-xs text-gray-400">系统预置</span> : <div className="flex justify-end gap-2"><AppButton size="sm" onClick={() => openEdit(item)}>编辑</AppButton><AppButton size="sm" variant="danger" onClick={() => void remove(item)}>删除</AppButton></div>
  ...
  return (
    <>
      <ResourcePage resourceKey="units" title="单位配置" description="物料只能选择已配置单位；自定义单位必须明确换算到所属计量方式的系统基准单位。" items={tableSort.sortedRows} getKey={(item) => `${item.measureType}-${item.code}`} columns={columns}
        ...
        actions={<ConfigurationManualOrder entity="units" label="单位" onMessage={onMessage} onSaved={load} />} viewMode={viewMode} onViewModeChange={setViewMode} displayModes={['card', 'list']} onCreate={openCreate} createLabel="新建单位" summary={<span className="text-sm text-gray-500">共 {filteredItems.length} 项</span>} rowLabel={(item) => `单位 ${item.name} ${item.code}`} />
