...
import { BookOpen, ExternalLink, Film, X } from 'lucide-react'
    ...
    <div className="fixed inset-0 z-[90] bg-slate-950/30" onClick={onClose}>
      <aside role="dialog" aria-modal="true" aria-label={`${pageLabel}操作帮助`} onClick={(event) => event.stopPropagation()} className="absolute inset-y-0 right-0 flex w-full max-w-xl flex-col bg-slate-50 shadow-2xl">
        <header className="flex items-center justify-between border-b border-slate-200 bg-white px-5 py-4">
          <div><div className="flex items-center gap-2 font-semibold text-slate-900"><BookOpen className="h-5 w-5 text-blue-600" />当前页面帮助</div><div className="mt-1 text-xs text-slate-500">{pageLabel} · {workflows.length} 个流程 · {videos.length} 个视频</div></div>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          ...
          {catalog && videos.length > 0 && <section className="space-y-3">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-800"><Film className="h-4 w-4 text-blue-600" />相关视频</div>
            {videos.map((video) => <SopVideoCard key={video.id} video={video} />)}
          </section>}
          {catalog && workflows.length === 0 && videos.length === 0 && <div className="rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm text-slate-500">当前页面暂时没有关联流程或视频，请进入完整帮助中心检索。</div>}
