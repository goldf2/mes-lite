...
export default function ProductionOrderModule({
  ...
  useEffect(() => { if (!selectedOrderBom || selectedOrderBomId === NO_BOM_VALUE) { if (selectedOrderCostRunId) setSelectedOrderCostRunId(''); return }; const eligibleRuns = selectedOrderBom.costRuns.filter((run) => !run.processRouteId || run.hasProcessRouteSnapshot); if (selectedOrderBom.costRuns.some((run) => run.id === selectedOrderCostRunId && (!run.processRouteId || run.hasProcessRouteSnapshot))) return; setSelectedOrderCostRunId(eligibleRuns[0]?.id || '') }, [selectedOrderBom, selectedOrderBomId, selectedOrderCostRunId])
      ...
      {mode === 'detail' && orderDetail && (
        <div className="rounded-lg bg-white p-6 shadow">
          ...
          <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-7">
            ...
            <InfoCard label="冻结工艺路线"><div className="font-medium">{orderDetail.processRouteName || '未关联'}</div><div className="text-xs text-gray-400">{orderDetail.processRouteSnapshot ? `${orderDetail.processRouteSnapshot.steps.length} 道工序` : '创建订单时未保存路线'}</div></InfoCard>
            <InfoCard label="冻结单位成本"><div className="font-medium">{orderDetail.bomCostSnapshot ? `${orderDetail.bomCostSnapshot.unitCost} 元` : '未计算'}</div><div className="text-xs text-gray-400">{orderDetail.bomCostSnapshot ? `总计 ${orderDetail.bomCostSnapshot.totalCost} 元` : '先完成 BOM 成本核价'}</div></InfoCard>
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-gray-200 p-4">
              ...
              <div className="grid gap-4 md:grid-cols-4">
                ...
                <label className="block text-sm font-medium text-gray-700">成本运行（可选）<SearchableSelect value={selectedOrderCostRunId} onChange={setSelectedOrderCostRunId} disabled={!selectedOrderBom || selectedOrderBomId === NO_BOM_VALUE} options={[{ value: '', label: selectedOrderBom?.costRuns.some((run) => !run.processRouteId || run.hasProcessRouteSnapshot) ? '自动取最新可用成本运行' : '当前 BOM 暂无可用成本运行' }, ...(selectedOrderBom?.costRuns || []).map((run) => ({ value: run.id, disabled: Boolean(run.processRouteId && !run.hasProcessRouteSnapshot), label: `${run.processRouteName || '默认路线'} · ¥${run.unitCost} · ${new Date(run.createdAt).toLocaleString('zh-CN')}${run.processRouteId && !run.hasProcessRouteSnapshot ? ' · 缺少冻结工艺，请重新计算' : ''}` }))]} placeholder={selectedOrderBom ? '选择已保存成本运行' : '请先选择 BOM'} /></label>
