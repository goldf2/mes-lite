...
export async function getProductionStatistics(input: { startDate?: string | null; endDate?: string | null; groupBy: string }) {
  const where = createdAtWhere(input)
  if (input.groupBy === 'worker') {
    const grouped = await prisma.workReport.groupBy({ by: ['workerName'], where, _sum: { goodQty: true, badQty: true }, _count: true })
  ...
  if (input.groupBy === 'status') {
    const grouped = await prisma.productionOrder.groupBy({ by: ['status'], where, _count: true })
    return grouped.map((item) => ({ status: item.status, orderCount: item._count }))
  }
  const grouped = await prisma.productionOrder.groupBy({
    by: ['productId'], where, _sum: { planQty: true, completeQty: true, scrapQty: true }, _count: true,
  })
...
export async function getQualityStatistics(input: { startDate?: string | null; endDate?: string | null }) {
  const where: Prisma.WorkReportWhereInput = createdAtWhere(input)
