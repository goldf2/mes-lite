...
export const salesOrderSearchFieldKeys = ['orderNo', 'voucherNo', 'status', 'customerId', 'material', 'orderDate', 'deliveryDate', 'totalAmount', 'currency', 'note', 'itemCount'] as const
export const shipmentSearchFieldKeys = ['shipmentNo', 'voucherNo', 'status', 'customerId', 'product', 'locationId', 'qty', 'unitPrice', 'totalAmount', 'customer', 'customerPhone', 'address', 'trackingNo', 'shippedBy', 'note', 'lotNo', 'shippedAt', 'createdAt'] as const
...
export function buildShipmentSearchCatalog(customers: readonly FulfillmentCustomer[], locations: readonly InventoryLocationOption[] = []) {
  return defineResourceSearchCatalog<Shipment>('shipment.actual-fields', [
    ...
    { key: 'note', label: '备注', type: 'text', read: (item) => item.note },
    { key: 'lotNo', label: '库存批次', type: 'text', read: (item) => item.items.flatMap((line) => line.lotAllocations.flatMap((allocation) => [allocation.lot.lotNo, allocation.lot.supplierLotNo])) },
...
export function buildReturnSearchCatalog(customers: readonly FulfillmentCustomer[], locations: readonly InventoryLocationOption[]) {
  return defineResourceSearchCatalog<ReturnOrder>('return.actual-fields', [
    ...
    { key: 'customerId', label: '客户', type: 'select', read: (item) => [item.shipment.customerId || '__UNASSIGNED__', item.shipment.customerRef?.code, item.shipment.customerRef?.name, item.shipment.customer], options: [{ value: '__UNASSIGNED__', label: '通用/未绑定' }, ...customers.map((customer) => ({ value: customer.id, label: `${customer.code} · ${customer.name}` }))] },
    { key: 'product', label: '退货物料', type: 'text', read: (item) => [item.shipmentItem.material.code, item.shipmentItem.material.name, item.shipmentItem.material.spec, item.product?.sku, item.product?.name] },
