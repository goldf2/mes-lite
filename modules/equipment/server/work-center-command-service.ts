...
export async function createManagedWorkCenter(input: WorkCenterInput) {
  return runWorkCenterCommand(() => prisma.$transaction(async (tx) => tx.workCenter.create({
    data: {
      ...
      machineRatePerHour: input.machineRatePerHour ?? 0,
      energyCostPerHour: input.energyCostPerHour ?? 0,
  ...
  return runWorkCenterCommand(() => prisma.$transaction(async (tx) => {
    ...
    const saved = await tx.workCenter.update({
      ...
      data: {
        ...
        ...(input.laborRatePerHour === undefined ? {} : { laborRatePerHour: input.laborRatePerHour }),
        ...(input.machineRatePerHour === undefined ? {} : { machineRatePerHour: input.machineRatePerHour }),
        ...(input.energyCostPerHour === undefined ? {} : { energyCostPerHour: input.energyCostPerHour }),
