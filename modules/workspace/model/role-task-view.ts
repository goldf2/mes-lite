...
export function buildRoleTaskSections(summary: RoleTaskSummary, permissions: PermissionMap): RoleTaskSection[] {
  ...
  if (canUpdate(permissions, 'shipmentDispatch')) warehouse.push({ key: 'shipment', label: '待发货', description: '复核客户、库位和实际批次', value: summary.pendingShipmentCount, tone: 'orange', functionKey: 'shipment', task: 'shipment' })
  if (canUpdate(permissions, 'returnReceive')) warehouse.push({ key: 'return', label: '退货待收货', description: '核对原发货单并转入待检', value: summary.pendingReturnCount, tone: 'red', functionKey: 'return', task: 'return' })
