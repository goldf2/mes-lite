...
export default function AiAgentSettingsPanel({ onMessage }: { onMessage: (message: string) => void }) {
      ...
      <section className="mt-8 border-t border-gray-200 pt-6" aria-labelledby="ai-agent-settings-title">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <AppButton variant="secondary" size="sm" onClick={() => setShowMarkLab(true)}><SlidersHorizontal aria-hidden="true" className="h-4 w-4" />图标调参</AppButton>
          ...
          <>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <label className="flex items-center justify-between gap-4 rounded-lg border border-gray-200 px-3 py-2.5 md:col-span-2">
                ...
                <input type="checkbox" checked={form.enabled} onChange={(event) => setForm((current) => ({ ...current, enabled: event.target.checked }))} className="h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
              </label>
              <label className="text-sm text-gray-700">提供商<select value={presetKey} onChange={(event) => { const preset = providerPresets.find((item) => item.key === event.target.value); if (preset) setForm((current) => ({ ...current, providerName: preset.providerName, baseUrl: preset.baseUrl })) }} className={`mt-1 ${appInputClassName}`}>{providerPresets.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}<option value="custom">自定义兼容服务</option></select></label>
              <label className="text-sm text-gray-700">提供商显示名称<input value={form.providerName} onChange={(event) => setForm((current) => ({ ...current, providerName: event.target.value }))} className={`mt-1 ${appInputClassName}`} maxLength={50} /></label>
              <label className="text-sm text-gray-700 md:col-span-2">OpenAI 兼容接口地址<input value={form.baseUrl} onChange={(event) => setForm((current) => ({ ...current, baseUrl: event.target.value }))} className={`mt-1 ${appInputClassName}`} placeholder="https://.../v1" spellCheck={false} /><span className="mt-1 block text-xs text-gray-500">生产环境仅接受 HTTPS，系统会自动拼接 `/chat/completions`。</span></label>
              <label className="text-sm text-gray-700 md:col-span-2">模型 ID<input value={form.model} onChange={(event) => setForm((current) => ({ ...current, model: event.target.value }))} className={`mt-1 ${appInputClassName}`} placeholder="填写已开通且支持工具调用的模型 ID" spellCheck={false} /></label>
              <label className="text-sm text-gray-700 md:col-span-2">API Key<span className="relative mt-1 block"><input type={showApiKey ? 'text' : 'password'} value={apiKey} onChange={(event) => setApiKey(event.target.value)} className={`${appInputClassName} pr-11`} placeholder={config.apiKeyConfigured ? '留空则继续使用现有密钥' : '输入新的 API Key'} autoComplete="new-password" disabled={!config.storageReady} spellCheck={false} /><button type="button" onClick={() => setShowApiKey((current) => !current)} aria-label={showApiKey ? '隐藏 API Key' : '显示 API Key'} title={showApiKey ? '隐藏 API Key' : '显示 API Key'} className="absolute inset-y-0 right-0 flex w-10 items-center justify-center text-gray-500 hover:text-gray-800 disabled:opacity-40" disabled={!config.storageReady}>{showApiKey ? <EyeOff aria-hidden="true" className="h-4 w-4" /> : <Eye aria-hidden="true" className="h-4 w-4" />}</button></span></label>
              {config.storedApiKeyConfigured && <label className="flex items-center gap-2 text-sm text-gray-600 md:col-span-2"><input type="checkbox" checked={clearStoredApiKey} onChange={(event) => setClearStoredApiKey(event.target.checked)} className="h-4 w-4 rounded border-gray-300 text-red-600 focus:ring-red-500" />清除页面保存的密钥{config.apiKeySource === 'ENV' ? '（清除后继续使用环境变量密钥）' : ''}</label>}
              <label className="text-sm text-gray-700">请求超时（毫秒）<input type="number" min={5000} max={120000} step={1000} value={form.timeoutMs} onChange={(event) => setForm((current) => ({ ...current, timeoutMs: Number(event.target.value) }))} className={`mt-1 ${appInputClassName}`} /></label>
              <label className="text-sm text-gray-700">最大工具轮次<select value={form.maxToolRounds} onChange={(event) => setForm((current) => ({ ...current, maxToolRounds: Number(event.target.value) }))} className={`mt-1 ${appInputClassName}`}>{[1, 2, 3, 4, 5, 6, 7, 8].map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
            </div>

            <div className="mt-5 flex flex-wrap justify-end gap-2 border-t border-gray-100 pt-4">
              <AppButton variant="secondary" onClick={testConnection} disabled={saving || testing || !config.configured}><PlugZap aria-hidden="true" className="h-4 w-4" />{testing ? '测试中' : '测试已保存配置'}</AppButton>
              <AppButton variant="primary" onClick={save} disabled={saving || testing}><Save aria-hidden="true" className="h-4 w-4" />{saving ? '保存中' : '保存 AI 配置'}</AppButton>
            </div>
          </>
        )}
      </section>
