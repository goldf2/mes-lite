...
export default function InventoryLotTraceDialog({ lotId, onClose, onMessage }: { lotId: string; onClose: () => void; onMessage: (message: string) => void }) {
  ...
  return (
    <ModalDialog title="批次正反向追溯" description="从当前批次查看来源原料、生产实绩、质检结论和下游产出。" onClose={onClose} size="wide" footer={<AppButton variant="primary" onClick={onClose}>关闭</AppButton>}>
      {loading && <AppLoadingIndicator compact label="正在加载批次谱系..." className="rounded-lg border border-gray-200 bg-gray-50" />}
      {!loading && data && <div className="grid gap-4 lg:grid-cols-[1fr_1.1fr_1fr]">
        <RelationList title={`上游投入批次 · ${data.upstream.length}`} empty="该批次没有上游生产投入；来料或历史兼容批次通常从这里开始。" items={data.upstream} onSelect={setSelectedId} />
        <section>
