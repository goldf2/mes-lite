...
MES-lite 默认采用“公共页面骨架 + 资源配置 + 领域扩展”的模块化实现。新增或修改功能时必须先复用现有控件、服务和业务能力，严格限制为单个页面重新实现同类结构。

- 开发前必须先使用代码搜索盘点现有实现，至少检查 `app/components/`、`lib/`、相邻 API 路由和 `docs/minierp/配置化页面框架规划.md`；不能在未检索的情况下直接新建控件或复制页面。
- 已有公共组件能够覆盖的按钮、字段、搜索选择器、工具栏、表格、卡片、弹窗、详情、分页、排序、文件查看和状态反馈，业务页面必须直接调用。缺少合理能力时优先扩展公共组件及其兼容接口，不得用局部复制品绕过。
- 常规资料的查询、新增、修改、归档和恢复默认接入统一资源页面骨架；页面只声明字段、筛选、权限、接口、关系和少量领域扩展。不得为供应商、客户、员工、设备、库位、单位等结构相近的资料分别复制完整 CRUD 页面。
...
- 新建独立页面骨架、控件或重复服务时，提交说明必须写明现有公共能力为何不能满足、为何不扩展公共模块、影响范围和后续复用边界；“开发方便”“改动较快”不是有效理由。
