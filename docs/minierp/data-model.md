...
BOM 数据保存“基准批量投入 -> 基准批量多项产出”。界面内置标准净用量、百分比损耗、固定损耗和批量换算四种录入模型；批量模型根据量纲显示为长度→件数、重量→件数、数量→数量等，但都按“投入总量 ÷ 主产出总量”归一为单位比例。保存时以主产出基准数量将界面单位用量还原为批量 `BOMItem.quantity`，不保存录入模型和计算参数。BOM 不保存生产批次额外损耗或实际耗用；生产订单实绩按订单冻结的 BOM 快照计算投入与各项产出，并为每项记录实际数量和去向库位。
