...
> 当前产品是单厂轻量 MES，使用 `prisma/schema.prisma` 的单实例模型；本文中的 `tenant_id`、商城、支付和多租户关系既未实现，也不是当前开发合同。当前物料主档、生产事实和 Product 外键退出规则以[单厂 MES 产品边界与核心模型收敛](../architecture/单厂MES产品边界与核心模型收敛.md)为准。`v0.1.357` 已在当前模型中交付来料/产出/客户退货内部批次、生产投入与产出谱系、发货与退货回流分配，以及待检、可用、冻结、返工、报废和完整质量处置；这些当前事实不并入本暂停草案的未来租户结构。
