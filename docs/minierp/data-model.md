...
`Material` / `Product` 是主数据，保存“这是什么”；`Stock` 是库存余额，保存“当前有多少”。两者不合并为一张表，页面和 API 负责把主数据与库存余额组合成库存清单。
...
`PATCH /api/stocks` 用于补齐缺失的物料/产品 0 库存余额记录，不修改已有库存数量、核算数量和金额。补齐动作使用幂等写入，重复执行不会改变已有库存。

库存页读取时若只发现缺失 0 库存余额，会自动调用补齐动作并重新加载；若发现负库存、预留大于库存、余额同时关联物料和产品等真正异常，仍会停止展示并列出明细。产品/物料资料更新、自动由物料生成 `MAT-*` 产品的接口也会同步补齐 0 库存余额，避免库存页被主数据缺失阻断。
