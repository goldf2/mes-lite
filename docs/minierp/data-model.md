...
附件缩略图不增加数据库字段：服务端根据 `storage_path + rotation` 计算确定性文件名，在上传或首次访问时生成 PNG 并保存在同一持久化附件目录。图片/PDF 原文件归档时保留缩略图以支持恢复，永久删除附件所属业务记录时同时删除原文件及其全部方向缩略图。
...
`BOMOutput.quantity` 保存同一基准批量的每项产出，并以 `isPrimary` 标识唯一主产出。每条物料投入 `BOMItem` 通过 `outputMaterialId` 明确绑定一项 `BOMOutput`，`BOMItem.quantity` 保存该产出基准数量对应的投入量，单位用量为 `BOMItem.quantity / 对应产出数量`。同一个投入物料可以分别绑定多个产出，数据库以“BOM + 投入物料 + 对应产出物料”保证唯一；每项产出至少要有一条投入转换关系。例如一批可同时保存“主产品 `100 件` + 可回收料 `0.2 kg`”，并分别定义两种产出各自需要的投入模型。投入物料不按 `Material.category` 限制，分类只参与筛选和显示，也可用成品或半成品作为投入表达二次加工。一对一只是普通数量比例，不设独立方案类型；同物料的纯移库不属于 BOM 转换。

### BOMItem 锯切成本组成

`BOMItem` 支持两类组成：

- `MATERIAL`：库存物料投入，可为原材料、半成品或已有产品；通过 `outputMaterialId` 归属于一项产出，生产订单实绩确认时从所选来源库位扣减并结转成本。
...
- `MATERIAL` 项的 `quantity` 保存对应产出的基准批量输入，`unit` 固定为所选投入物料主库存单位。当前成本试算只展开绑定主产出的投入项，先除以主产出数量得到单位用量，再乘数量基准和库存成本单价；其他产出的成本分摊仍是后续切片。
- `SAWING_COST` 或其他成本对象项读取生效成本版本，按数量计算材料成本、人工工时、机时和直接费用。
- 固定费用作为本次 `OVERHEAD` 快照行保存，不写入 BOM 本体。

BOM 数据保存“每项产出的基准批量投入 -> 基准批量多项产出”。界面会先选定当前产出，再为该产出单独提供“单位标准用量”和“投入总量 ÷ 产出总量”两种内置录入模型；批量模型根据量纲显示为长度→件数、重量→件数、数量→数量等，最终都归一为当前产出的单位投入比例。保存时以当前产出基准数量将界面单位用量还原为 `BOMItem.quantity`，并保存 `outputMaterialId`，不保存录入模型和计算参数。标准 BOM 不再单独设置固定或百分比损耗：废料、废屑和可回收料必须作为明确的 `BOMOutput` 记录，避免与损耗字段重复核算。生产订单实绩按订单冻结的各产出实际数量分别计算其投入贡献，再按投入物料汇总扣库，并为每项产出记录实际数量和去向库位；实绩中的额外耗用仅表示本批次计划外差异。
