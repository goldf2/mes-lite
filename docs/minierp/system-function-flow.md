...
- 生产只对主工序派工；辅助流程、在线检测和现场返工先随报工记录，不单独派单。
...
当前采用轻量生产模式：主工序派工；在线检测、现场返工、辅助流程随报工记录，不强制形成独立单据，也不单独派单。
...
- 辅助流程同样先作为工艺/报工结果记录，不进入派工管理；后续再按需要单据化。
...
| P1 | 工序类型 | 区分主工序、辅助流程、检验点；只有主工序生成派工 |
