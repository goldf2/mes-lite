# MES-lite 系统功能与流程总览

本文用于统一当前系统建模口径，并标出后续需要补齐的功能。

## 核心建模原则

- 所有可库存、可采购、可生产、可销售、可报废、可回收的对象统一称为“物料”。
- 当前默认启用轻量生产业务模式：仪表盘、生产日报、物料、来料、工单、库存、发货、退货作为主线入口。
- 派工和 BOM 成本计算暂时隐藏，待日常流转稳定后再逐步打开。
- 原材料、成品、辅材、废料、废品是物料分类，不是独立主数据体系。
- 待检、可用、冻结、在制、报废待处理是库存状态，不是物料分类。
- 来料红冲保持整单冲销；非整单反向业务后续通过独立来料退货/冲销单处理。
- 普通数量、重量、金额尾差通过库存调整处理，并写入库存流水和操作记录。
- 工序类型和是否派单分开配置；派单节点进入派工管理，不派单节点只随报工记录。

## 总体功能图

```mermaid
mindmap
  root((MES-lite))
    主数据
      统一物料
        原材料
        成品
        辅材
        废料
        废品
      供应商
      BOM
      工艺路线
      仓库与状态
    来料管理
      来料单
      原始单据附件
      收货
      拒收
      整单红冲
      来料退货单_待补
    库存管理
      库存查询
      双单位库存
      库存金额
      FIFO成本层
      库存调整
      库存流水_待增强
      仓库状态库存_待补
    生产日报
      每日产量
      不良品
      报废品
      原材料使用
      生产人员
    成本计算
      锯切成本
      成本对象
      BOM成本快照
    生产管理
      工单
      领料
      派工
      报工
      在线检测
      现场返工
      成品入库
      单据化返工_后续
    发货管理
      发货单
      送货单PDF
      签收
      成品退货
    退货与冲销
      成品退货
      来料退货_待补
      报废处理_待补
      转废料_待补
    权限与审计
      操作人员注册
      人员权限
      权限组
      操作记录
      归档恢复
    移动端
      小程序首页
      库存查询
      移动入库
      移动盘点
      移动报工_待补
```

## 统一物料模型

```mermaid
flowchart LR
  M["物料主数据"] --> C["物料分类"]
  C --> C1["原材料"]
  C --> C2["成品"]
  C --> C3["辅材"]
  C --> C4["废料"]
  C --> C5["废品"]

  M --> A["业务属性"]
  A --> A1["可采购"]
  A --> A2["可生产"]
  A --> A3["可销售"]
  A --> A4["可领料"]
  A --> A5["可回收"]

  M --> U["双单位"]
  U --> U1["库存单位"]
  U --> U2["核算单位"]
  U --> U3["换算率"]

  M --> K["成本方法"]
  K --> K1["移动加权"]
  K --> K2["FIFO"]
```

当前系统仍存在 `Material` 与 `Product` 两套主数据。后续应逐步合并为统一物料，成品由物料分类表达。

## 库存模型

建议目标模型：

```mermaid
flowchart LR
  I["库存"] --> M["物料"]
  I --> W["仓库"]
  I --> L["库位_可选"]
  I --> S["库存状态"]
  I --> Q["库存数量"]
  I --> V["核算数量"]
  I --> A["库存金额"]

  S --> S1["待检"]
  S --> S2["可用"]
  S --> S3["冻结"]
  S --> S4["在制"]
  S --> S5["报废待处理"]
```

第一阶段建议先做“仓库 + 库存状态”，库位细分后置。

## 来料流程

```mermaid
flowchart TD
  A["创建来料单"] --> B{"是否收货"}
  B -->|拒收| C["来料单状态: 已拒收"]
  B -->|收货| D["库存增加"]
  D --> E["生成库存流水"]
  D --> F["生成成本层"]
  F --> G["来料单状态: 已收货"]
  G --> H{"是否需要冲销"}
  H -->|整单错误| I["整单红冲"]
  H -->|部分退料| J["来料退货/冲销单_待补"]
  H -->|数量金额尾差| K["库存调整"]
```

现状：

- 已有来料单、收货、拒收、整单红冲、附件。
- 已有旧数据兼容红冲和库存调整。
- 缺少独立来料退货/冲销单。

## 生产日报流程

当前轻量模式不强制先派工，只要求每天把产量、报废、不良、原材料使用和生产人员记录准确；需要时仍可用工单记录计划、领料、报工和入库。

```mermaid
flowchart TD
  A["选择日期 / 班次"] --> B["录入产品产量"]
  B --> C["录入不良品和报废品"]
  C --> D["记录原材料使用"]
  D --> E["记录生产人员"]
  E --> F["生成生产日报"]
  F --> G["更新统计看板"]
  D --> H["核对物料库存"]
```

## 可选派工生产流程

工单流程保留在轻量业务菜单中，派工管理暂时隐藏。需要完整追踪负责人、计划时间、工序进度和责任时，再打开派工。

```mermaid
flowchart TD
  A["创建工单"] --> B["确认工单"]
  B --> C["领料"]
  C --> D{"工序是否派单"}
  D -->|是| E["生成派工单"]
  E --> F["派工/开工/完工"]
  F --> G["报工"]
  D -->|否| G
  G --> H["在线检测或流程记录"]
  H --> I{"是否合格"}
  I -->|合格| J{"是否最后工序"}
  I -->|可返工| K["现场返工记录"]
  K --> H
  I -->|不可返工| L["记录废品/废料"]
  J -->|否| D
  J -->|是| M["成品入库"]
  L --> N["库存调整或报废处理_后续增强"]
```

现状：

- 已有工单、领料、派工、报工、成品入库基础能力。
- 现场检测和返工可以先随报工记录表达，不强制生成质检单或返工单。
- 辅助流程默认只作为工艺/报工结果记录；如果打开派单开关，则进入派工管理。
- 待检库存、返工单、报废处理单属于后续增强，不作为当前简单生产的前置条件。

## BOM 成本计算流程

BOM 成本计算是独立成本测算，不联动派工。它用于算产品结构成本和报价依据，不改变库存和工单状态。

```mermaid
flowchart TD
  A["维护成本对象"] --> B["录入单位材料成本、人工时、机时、直接费"]
  B --> C["成本对象可加入产品 BOM"]
  D["选择产品"] --> E["读取产品 BOM"]
  C --> E
  E --> F{"BOM 项类型"}
  F -->|MATERIAL| G["读取物料库存成本"]
  F -->|成本对象| H["读取生效成本对象成本"]
  G --> I["按数量和损耗计算材料成本"]
  H --> J["按数量计算材料、人工时、机时和直接费"]
  I --> K["汇总 BOM 成本"]
  J --> K
  K --> L["录入人工费率、机时费率、固定费用分摊"]
  L --> M["保存 BomCostRun 快照"]
  M --> N["回看历史成本明细"]
```

边界：

- `MATERIAL` BOM 项参与工单领料、库存预留和库存扣减。
- `SAWING_COST` / 成本对象 BOM 项只参与成本结构和 BOM 成本计算。
- 固定费用分摊只写入本次成本快照，不写回 BOM。
- 派工仍由工艺路线和工单驱动，不由 BOM 成本快照驱动。
- “成本数据”页签用于检查成本对象、产品 BOM、默认工艺路线、可计算工艺和最近成本快照；物料全景图同步展示该物料相关加工参数和成本对象。

### 派单与不派单逻辑

| 模式 | 适用场景 | 操作逻辑 |
| --- | --- | --- |
| 派单 | 需要追踪负责人、计划数量、计划时间、进度和责任的工序 | 工单领料后生成派工单，执行派工、开工、完工，再报工 |
| 不派单 | 辅助流程、在线检测、简单返工、临时处理 | 不进入派工管理，只在报工时记录数量、质量结果、备注和附件 |

## 报废转废料流程

```mermaid
flowchart TD
  A["原物料或成品"] --> B["不合格/报废待处理"]
  B --> C{"处理方式"}
  C -->|返工| D["进入返工流程"]
  C -->|降级| E["转为可用但降级标识"]
  C -->|销毁| F["库存减少并计损耗"]
  C -->|转废料| G["报废处理单"]
  G --> H["原物料库存减少"]
  G --> I["废料物料库存增加"]
  G --> J["差额计入损耗"]
```

关键判断：

- 报废待处理时，仍然是原物料。
- 只有确认转废料后，才变成新的废料物料。

## 发货与成品退货流程

```mermaid
flowchart TD
  A["成品可用库存"] --> B["创建发货单"]
  B --> C["确认发货"]
  C --> D["成品库存减少"]
  C --> E["生成送货单PDF"]
  D --> F{"客户是否退货"}
  F -->|否| G["流程结束"]
  F -->|是| H["成品退货单"]
  H --> I["退货待检"]
  I --> J{"处理结果"}
  J -->|合格回库| K["成品可用库存增加"]
  J -->|不合格| L["报废待处理"]
```

现状：

- 已有发货单、确认发货、送货单 PDF、成品退货基础能力。
- 成品退货后的待检/报废状态需要继续增强。

## 库存调整流程

```mermaid
flowchart TD
  A["发现账实差异"] --> B{"差异来源"}
  B -->|来料整单错误| C["来料整单红冲"]
  B -->|业务退货| D["退货/冲销单_待补"]
  B -->|盘点差异| E["库存调整"]
  B -->|称重/小数尾差| E
  B -->|早期数据成本缺失| E
  E --> F["调整数量/核算数量/库存金额"]
  F --> G["写入库存流水"]
  F --> H["写入操作记录"]
```

库存调整不是业务退货单据，只用于账实修正、盘点差异、损耗和历史数据尾差。

库存页会做一致性检查：物料或产品主数据必须有对应 `Stock` 余额行。`Material` / `Product` 仍是主数据表，`Stock` 是库存余额表，库存页只是在展示时把两者组合成库存清单。

若只缺少 0 余额行，具备库存修改权限的人员打开库存页时会自动调用“补齐库存余额”动作创建缺失的物料/产品库存记录；该动作不调整已有数量和金额，只补主数据对应余额行并写操作记录。若自动补齐失败，页面会显示手动补齐入口；若当前人员没有库存修改权限，页面会保留一致性异常提示，等待有权限人员处理。若存在负库存、预留大于库存或关联异常，则停止展示并列出需要人工处理的明细。

## 权限与审计流程

```mermaid
flowchart TD
  A["操作人员注册"] --> B["管理员审核"]
  B --> C["人员权限/权限组"]
  C --> D["功能访问控制"]
  D --> E["新增/修改/归档/恢复/调整"]
  E --> F["操作记录"]
  E --> G["业务附件"]
```

## 当前已具备功能

- 登录注册、人员审核、权限组、人员权限。
- 物料管理，支持图片、多图封面、归档。
- 来料管理，支持双单位、报价方式、收货、拒收、整单红冲。
- 库存查询，支持数量、核算数量、库存金额、单位成本。
- 库存调整，支持数量、核算数量、库存金额调整。
- FIFO 成本层与领料消耗。
- 工单、领料、派工、报工、成品入库。
- 发货、送货单 PDF、成品退货基础流程。
- 操作记录、归档记录、附件。
- Coolify 部署与持久化目录。

## 需要优先补齐的功能

| 优先级 | 功能 | 目的 |
| --- | --- | --- |
| P0 | 统一物料主数据 | 将 Product 合并为物料分类=成品 |
| P0 | 仓库与库存状态 | 区分待检、可用、在制、报废待处理 |
| P0 | 来料退货/冲销单 | 支持非整单退料，不再直接改来料单 |
| P0 | 库存流水页面 | 人工核查每次库存变化 |
| P1 | 工序类型与派单开关 | 区分主工序、辅助流程、检验点，并配置是否生成派工单 |
| P1 | 报工质量结果 | 在报工中记录合格、返工、废品、废料数量 |
| P1 | 废品/废料入账 | 将报工产生的废品、废料进入对应物料库存 |
| P2 | 报废处理单 | 支持复杂报废、返工、转废料、损耗 |
| P2 | 在制品流转 | 工序间需要精细追踪时再启用 |
| P2 | 成品待检与质检单 | 需要独立质检部门或批量检验时再启用 |
| P1 | 仓库管理 | 原材料仓、成品仓、待检仓、废料仓 |
| P2 | 库位管理 | 后续按区域、货架、箱号管理 |
| P2 | 小程序移动报工/盘点 | 现场操作闭环 |

## 下一步建议

1. 先补“库存流水页面”，方便继续测试和查账。
2. 再补“仓库 + 库存状态”，先不做细库位。
3. 然后做“来料退货/冲销单”，把非整单反向业务从红冲里分离。
4. 最后做“统一物料主数据”，迁移 Product 到 Material。
