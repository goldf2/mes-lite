...
## 2026-07-29 铝型材定制补充审计

当前结构无需推倒重写。`Material + Stock + StockLog + InventoryCostLayer` 可继续作为规格、数量、双单位和成本总账；为解决同规格不同实际长度，新增独立实体子账，不把实际长度塞入普通 BOM。

第一条垂直切片采用 `ProfileSpec`、`MaterialInProfileLine`、`ProfileStockEntity`、`ProfileStockMovement` 和 `ManufacturingConfig`。

兼容约束：

1. 历史库存不伪造实际长度，允许显示为“未实体化余额”。
2. 新来料有实测行时同时写总账和子账；普通来料仍走旧流程。
3. 已被拆分或使用的来源实体不能随来料单直接红冲。
4. 旧生产日报和发货流程继续使用规格总账，直到后续切割闭环接入实体领用。
5. 生产日报已有 BOM 快照；生产工单仍缺少完整 BOM/工艺快照，列入阶段 2/3 前置工作。

完整差距与目标数据流见 [铝型材定制MES阶段0审计.md](./铝型材定制MES阶段0审计.md)，技术决策见 [ADR 0002](../adr/0002-profile-entity-subledger.md)。
...
当前入口已收敛为轻量生产业务模式：顶部一级菜单按工作台、物料、生产、物流、库存、工具划分，左侧显示当前一级分类下的功能页面。“物料”下设物料管理、BOM 设定和 BOM 反查三个入口；“工具”当前包含锯切成本和硬件工具。硬件工具先独立提供 Honeywell 1900 扫码计数与 PC310T 标签测试打印，不与发货单、库存或生产状态联动。派工管理作为隐藏高级模块，不再作为小微企业第一阶段主流程入口。
...
7. 软删除方向基本一致，符合“原则上不物理删除”的要求；`Product` 已退为内部兼容表，后续物理迁移外键时再处理其生命周期字段。
