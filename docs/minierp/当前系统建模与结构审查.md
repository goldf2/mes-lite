...
当前产品入口已收敛为轻量生产业务模式：默认业务菜单保留仪表盘、生产日报、物料管理、BOM关系、来料管理、工单管理、库存管理、发货管理、退货管理和锯切成本。BOM 第一阶段用于显示和维护成品与原材料关系；派工管理作为隐藏高级模块，不再作为小微企业第一阶段主流程入口。
...
| `BOM` / `BOMItem` | 产品组成清单 | 绑定 `Product`；`MATERIAL` 项消耗 `Material`，成本对象项只参与成本计算 |
