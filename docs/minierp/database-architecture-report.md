...
- 来料单头记录供应商、凭据、统一待分库库位和状态；每条物料明细记录主单位实收量、一个辅助单位实测/历史推算量、来源、样本数、单位版本、计价、金额和批次。旧件数/总长/总重列只保留兼容。
...
| 已有库位，但没有仓库和库存状态 | 可定位实物库位，待检品、在制品、废料状态仍只能临时表达 | 后续按真实需求增加仓库与库存状态维度 |
