...
`v0.1.371` 已在完整批次闭环、岗位任务和数据范围基础上，把设备运行事件、点检和维保独立为第 49-51 个权限资源；设备维护组可执行事件命令、点检和维保，备件领用同时受库位范围保护，其他岗位需单独授权。生产、点检和维保任务按本人/工作中心范围，库存与质量任务按库位范围过滤，个人例外使用有期限临时授权。部门/班组范围仍未独立建模。
