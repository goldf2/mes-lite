...
| 设备点检、保养和维修工单 | 后续 P1 | 先做计划、记录和到期提醒，不扩展财务资产管理 |
| PLC、OEE、APS 和专业 WMS | 暂不建设 | 仅在现场需求和采集条件明确后评估 |

## 5. 下一阶段最小闭环

`v0.1.369` 已在完整批次闭环、岗位任务和数据范围基础上，把设备运行事件独立为第 49 个权限资源；设备维护组可执行开停机、故障和恢复，其他设备只读岗位只能回放。生产任务按本人/工作中心范围、库存与质量任务按库位范围过滤，个人例外使用有期限临时授权。部门/班组范围仍未独立建模。
