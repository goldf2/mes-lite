...
本文用泳道图说明轻量 MES 从需求、计划、领料、生产、检验到入库和发货的责任交接，并明确人工角色与系统自动动作。图中包含当前已经具备的能力和后续需要补齐的质量、批次与异常闭环，不代表所有节点都已上线。
...
| 来料、库位库存、库存流水 | 已有基础能力 | 库存流水已经可以独立查询，但尚缺统一库存状态维度 |
| 生产质量判定 | 下一阶段 P0 | 需要把生产实绩、待检、判定和库存放行串成闭环 |
| 批次正反向追溯 | 下一阶段 P0 | 需要统一批次实体及投入、产出、发货谱系 |
| 不合格处置 | 后续 P1 | 需要返工、报废、退供应商和让步接收闭环 |
| 设备事件、点检和保养 | 后续 P1 | 先做事件记录和到期提醒，不扩展财务资产管理 |
| PLC、OEE、APS 和专业 WMS | 暂不建设 | 仅在现场需求和采集条件明确后评估 |

## 5. 下一阶段最小闭环

下一阶段只实施“生产实绩 → 待检 → 合格/不合格 → 可用/冻结库存 → 库存流水”，不一次建设完整质量管理系统：

1. 检验状态只保留待检、合格和不合格。
2. 检验记录只包含实绩来源、结论、人员、时间、备注和附件。
3. 合格转为可用库存，不合格转为冻结库存。
4. 所有状态转换由服务端事务校验，并写库存流水与审计记录。
