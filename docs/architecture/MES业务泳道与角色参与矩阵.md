...
| 来料、库位库存、库存流水 | 部分闭环 | 生产产出已有待检/可用/冻结状态；来料和其他来源仍默认可用，仓库层级尚未建立 |
| 生产质量判定 | 第一切片已交付 | 生产实绩、产出批次、待检任务、整批放行/冻结和库存流水已串联；部分判定和处置待补 |
| 批次正反向追溯 | 第一切片已交付 | 已有生产产出内部批次和状态交易；投入、来料、发货批次及父子谱系待补 |
...
`v0.1.354` 已实施“生产实绩 → 产出内部批次 → 待检 → 整批合格/不合格 → 可用/冻结库存 → 库存流水”。接下来仍不一次建设完整质量管理系统，而按以下顺序深化：
