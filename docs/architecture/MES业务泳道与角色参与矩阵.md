...
| 来料、库位库存、库存流水 | 批次级闭环已交付 | 来料收货生成内部批次；有已发布来料标准时自动隔离并建检验任务，无标准时直接可用；流程转移同步批次库位余额，仓库层级尚未建立 |
| 生产/退货/来料质量检验 | 标准化批次闭环已交付 | 标准版本、自动抽样、任务项目快照、逐项判定、附件、趋势、复检、返工、报废、授权放行和库存流水已串联；SPC 与多级审批待补 |
...
`v0.1.374-v0.1.375` 已在完整批次闭环、岗位任务和数据范围基础上，将来料收货/红冲及七个物流状态动作拆为独立命令资源。质量检验组只读生效标准并判定；普通仓管执行收货、发货、退货入库和正向移库，仓库主管另执行有原因红冲、取消、拒绝和冲销，销售跟单负责签收及销售侧取消/拒绝。仓库命令不能替代质量任务读取或判定，通用单据编辑也不能替代库存过账。生产、点检和维保任务按本人/工作中心范围，库存与质量任务、附件和趋势按库位范围过滤。部门/班组范围仍未独立建模。
...
5. 检验标准、抽样、逐项结果、附件、描述性趋势和按标准启用的来料自动送检已交付；暂不增加 SPC/过程能力、复杂审批和自动设备采集。
