...
`v0.1.373` 已在完整批次闭环、岗位任务和数据范围基础上，让已发布来料标准驱动仓库收货后的隔离与质量任务交接；设备运行事件、点检、维保和检验标准仍使用第 49-52 个权限资源。质量检验组只读生效标准并判定，质量处置组维护标准，仓库组只能收货而不能借此读取或判定质量任务。生产、点检和维保任务按本人/工作中心范围，库存与质量任务、附件和趋势按库位范围过滤。部门/班组范围仍未独立建模。
