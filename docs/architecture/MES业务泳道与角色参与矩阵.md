...
| 设备保养和维修工单 | 后续 P1 | 先做计划、执行、备件与关闭证据，不扩展财务资产管理 |
| PLC、OEE、APS 和专业 WMS | 暂不建设 | 仅在现场需求和采集条件明确后评估 |

## 5. 下一阶段最小闭环

`v0.1.370` 已在完整批次闭环、岗位任务和数据范围基础上，把设备运行事件和设备点检独立为第 49/50 个权限资源；设备维护组可执行事件命令、建立并执行点检，其他岗位需单独授权。生产与点检任务按本人/工作中心范围、库存与质量任务按库位范围过滤，个人例外使用有期限临时授权。部门/班组范围仍未独立建模。
