...
`v0.1.374` 已在完整批次闭环、岗位任务和数据范围基础上，让已发布来料标准驱动仓库收货后的隔离与质量任务交接，并以第 53-54 个资源拆分来料收货/拒收与红冲。质量检验组只读生效标准并判定，质量处置组维护标准，普通仓库组只能收货，仓库主管才可有原因红冲；仓库命令不能替代质量任务读取或判定。生产、点检和维保任务按本人/工作中心范围，库存与质量任务、附件和趋势按库位范围过滤。部门/班组范围仍未独立建模。
