...
本文只描述当前代码已经具备的业务能力，不混入尚未交付的序列号、来料自动检验、SPC/过程能力、自动设备采集或 OEE。需要了解未来目标流程时，再查看《MES 业务泳道与角色参与矩阵》。

> `v0.1.372` 补充：来料、生产投入/产出、客户发货和退货回流均已形成内部批次关系；生产/退货质量任务支持标准版本、自动抽样、任务快照、逐项结果、附件、趋势和完整处置，批次追溯可多跳展开。岗位任务、生产命令、数据范围和临时授权已落地，业务资源现为 52 项；设备事件、周期点检、保养维修与备件库存/成本/批次领用已贯通。
...
1. 生产/退货质量已覆盖标准版本、自动抽样、任务项目快照、逐项判定、附件、趋势及全部处置；来料自动检验、SPC/过程能力、原因分类和复杂审批流尚未建设。
