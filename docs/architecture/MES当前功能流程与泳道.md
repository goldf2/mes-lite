...
> `v0.1.409` 补充：来料、生产投入/产出、客户发货和退货回流均已形成内部批次关系；生产、退货及按已发布标准启用的来料质量任务支持标准版本、自动抽样、任务快照、逐项结果、附件、趋势和完整处置，批次追溯可多跳展开。班后实绩冻结实际设备与作业文件版本；岗位任务、生产命令、来料与物流状态命令、数据范围和临时授权已落地，业务资源现为 61 项；设备事件、周期点检、保养维修与备件库存/成本/批次领用已贯通。发货单可创建唯一箱码的货箱单据，记录内容物、打包责任人、时间、重量、封箱号和现场照片，并通过发货单码/货箱码返回唯一系统单据。
...
flowchart LR
  ...
  Finished --> Package["货箱单据<br/>内容物 + 打包照片 + 箱码"]
  SO --> Shipment["发货单<br/>发货单码 + 物流号"]
  Package --> Shipment
  Shipment --> DispatchCheck{"已开始装箱？"}
  DispatchCheck -->|"是：货箱数量完整"| Customer["发货确认 / 客户收货<br/>按批次扣减"]
  DispatchCheck -->|"否：兼容历史单据"| Customer
...
客户资料 → 销售订单记录需求；另由发货单登记一个客户和多条实际发货物料 → 创建货箱单据、上传实物/打包现场照片并打印箱码 → 装箱完整性校验 → 打印发货单码并保持物流运单号独立 → 确认发货并按明细和批次扣减库存 → 从原发货明细登记退货 → 收货进入独立待检批次 → 整批/部分判定 → 必要时复检、返工、报废或授权放行。

销售订单保存客户需求、物料、数量、单价和交期；订单卡片和详情中的物料名称、编码可进入只读物料详情，关闭后恢复订单上下文。发货不保存订单或订单明细 ID，也不回写订单数量和状态；未发/超发按“客户 + 物料”动态汇总供参考。新发货单可根据实际包装拆成多个货箱，货箱和发货单分别有唯一可扫码编码；未开始货箱管理的历史发货单保持旧流程兼容。销售订单目前不会自动生成生产订单，两者由业务人员按实际需求衔接。
...
仓库全景是该事实链的只读空间投影：按当前账号授权库位，把库存余额映射为二维库位卡片，并按可用、待检、冻结和返工数量着色；它不执行库存转移，也不保存另一套余额。需要修改库存时仍返回库存管理或既有受控业务单据。
