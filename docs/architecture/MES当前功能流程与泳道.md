...
本文只描述当前代码已经具备的业务能力，不混入尚未交付的序列号、检验模板/附件、设备点检/保养或 OEE。需要了解未来目标流程时，再查看《MES 业务泳道与角色参与矩阵》。

> `v0.1.369` 补充：来料、生产投入/产出、客户发货和退货回流均已形成内部批次关系；质量任务支持完整处置，批次追溯可多跳展开。岗位任务、生产命令、数据范围和临时授权已落地，业务资源现为 49 项；设备开机、停机、故障、恢复及持续时间已形成事件时间线。
...
- 设备与工作中心已经具备台账和基础配置；运行状态只能通过开机、停机、故障、恢复事件改变，事件保留原因、人员、时间和故障/停机持续时间。点检、保养、维修工单、自动采集和 OEE 尚未贯通。
