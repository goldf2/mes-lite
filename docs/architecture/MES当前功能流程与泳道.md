...
本文只描述当前代码已经具备的业务能力，不混入尚未交付的序列号、通用检验模板、自动设备采集或 OEE。需要了解未来目标流程时，再查看《MES 业务泳道与角色参与矩阵》。

> `v0.1.371` 补充：来料、生产投入/产出、客户发货和退货回流均已形成内部批次关系；质量任务支持完整处置，批次追溯可多跳展开。岗位任务、生产命令、数据范围和临时授权已落地，业务资源现为 51 项；设备事件、周期点检、保养维修与备件库存/成本/批次领用已贯通。
