...
本文只描述当前代码已经具备的业务能力，不混入尚未交付的序列号、检验模板/附件、批次搜索全景和设备事件。需要了解未来目标流程时，再查看《MES 业务泳道与角色参与矩阵》。

> `v0.1.361` 补充：来料、生产投入/产出、客户发货和退货回流均已形成内部批次关系；质量任务支持完整处置，批次追溯可多跳展开。仪表盘按服务端权限和人员数据范围优先呈现岗位待办，生产命令分别授权，业务、配置和运维资源已细分为 48 项；生产支持本人/工作中心范围，库存支持库位范围，个人临时授权具备原因、授权人和有效期。
