...
客户资料 → 销售订单 → 创建或关联发货单 → 创建货箱单据、上传实物/打包现场照片并打印箱码 → 装箱完整性校验 → 打印发货单码并保持物流运单号独立 → 确认发货并按批次扣减库存 → 从原发货登记退货 → 收货进入独立待检批次 → 整批/部分判定 → 必要时复检、返工、报废或授权放行。

销售订单保存物料、数量、单价、交期和已发数量；订单卡片和详情中的物料名称、编码可进入只读物料详情，关闭后恢复订单上下文。发货可以独立登记，也可以关联销售订单及其明细。新发货单可根据实际包装拆成多个货箱，货箱和发货单分别有唯一可扫码编码；未开始货箱管理的历史发货单保持旧流程兼容。销售订单目前不会自动生成生产订单，两者由业务人员按实际需求衔接。
