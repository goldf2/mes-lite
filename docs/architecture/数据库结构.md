...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.356`。当前运行库为 SQLite；暂停的租户化草案不代表当前实现或当前产品合同。`v0.1.354` 新增生产产出批次和质量状态，`v0.1.355` 连接来料、生产投入与产出谱系，`v0.1.356` 继续连接客户发货、退货回流和退货质检批次。详见[发货退货批次追溯与质检操作](../operations/发货退货批次追溯与质检操作.md)。
...
| `QualityInspection` | 生产产出或客户退货待检及判定 | 新建为 `PENDING`；整批 `PASS` 或 `FAIL` 后成为 `COMPLETED`，保存抽样与判定数据 |
...
当前停止线是“供应商批号/来料内部批次 → 流程转移 → 生产投入 FIFO 分配 → 产出内部批次 → 质检 → 发货批次/客户 → 原发货退货 → 独立退货待检批次 → 放行/冻结”。它是批次级正反向追溯，不是逐件序列号；部分放行、复检、让步、返工、报废和解冻仍未实现。
