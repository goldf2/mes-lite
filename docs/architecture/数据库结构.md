...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.364`。当前运行库为 SQLite；暂停的租户化草案不代表当前实现或当前产品合同。`v0.1.354-v0.1.356` 依次建立生产质量批次、来料到生产谱系和客户发货/退货回流，`v0.1.357` 增加完整不合格处置，`v0.1.358` 提供跨业务批次搜索与多跳全景，`v0.1.359-v0.1.360` 交付岗位任务、命令权限、48 个细分资源和岗位预置组，`v0.1.361` 增加人员数据范围与个人临时授权元数据；`v0.1.362` 增加 CI 与商业运维交付基线，`v0.1.363-v0.1.364` 修正 GitHub Actions 的数据库 URL 与历史提交检出，不修改 Schema。操作基线见[全流程作业指导书](../operations/user-guide/MES-lite全流程作业指导书-v0.1.362.md)。当前共有 77 个 Prisma 模型和 76 个迁移。
