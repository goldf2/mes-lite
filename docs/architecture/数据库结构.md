...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.344`。当前运行库为 SQLite，模型设计通过 Prisma 保持向 PostgreSQL 迁移的可能性；`docs/minierp/data-model.md` 中的租户字段属于目标草案，不代表当前已实现。`v0.1.344` 为 BOM 增加受控生命周期、版本派生与数据库不可变约束；历史启用 BOM 迁移为已发布，历史停用 BOM 迁移为已作废。
