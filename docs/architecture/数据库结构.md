...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.350`。当前运行库为 SQLite；暂停的租户化草案不代表当前实现或当前产品合同。`v0.1.349` 冻结 Product 和旧生产写入；`v0.1.350` 不改变 Schema 或批量数据，只将新 Material 工单的应用层状态收敛为 `DRAFT → RELEASED → IN_PROGRESS → COMPLETED/CANCELLED`，并在读取时归并旧状态。Product 外键退出表、迁移顺序和状态规则见[单厂 MES 产品边界与核心模型收敛](./单厂MES产品边界与核心模型收敛.md)。
