...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.349`。当前运行库为 SQLite；暂停的租户化草案不代表当前实现或当前产品合同。`v0.1.349` 不改变 Schema 和数据，只冻结兼容写入：库存完整性只面向有效 `Material`，不再创建 Product 独占库存；旧生产日报禁止新建；旧领料、报工和入库只允许 `materialId=null` 的历史工单。Product 外键退出表、迁移顺序和目标状态机见[单厂 MES 产品边界与核心模型收敛](./单厂MES产品边界与核心模型收敛.md)。
