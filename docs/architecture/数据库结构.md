...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.370`。当前运行库为 SQLite；暂停的租户化草案不代表当前实现或当前产品合同。`v0.1.354-v0.1.358` 建立质量、批次、发货退货和多跳全景，`v0.1.359-v0.1.361` 交付岗位任务、48 个细分资源、人员数据范围和临时授权；`v0.1.362-v0.1.368` 建立 CI、SOP、恢复、归档授权与 Product→Material 预检，`v0.1.369-v0.1.370` 新增设备事件、周期点检与第 49/50 个权限资源。操作基线见[全流程作业指导书](../operations/user-guide/MES-lite全流程作业指导书-v0.1.370.md)。当前共有 82 个 Prisma 模型和 78 个迁移。
