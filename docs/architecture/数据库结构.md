...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.346`。当前运行库为 SQLite，模型设计通过 Prisma 保持向 PostgreSQL 迁移的可能性；`docs/minierp/data-model.md` 中的租户字段属于目标草案，不代表当前已实现。`v0.1.346` 为 `Operator` 增加密码失败锁定字段，并新增 `AuthenticationThrottle` 保存登录、注册和管理员安装的来源限流窗口；迁移只增加列和表，不改写既有账号、角色、密码或会话。
