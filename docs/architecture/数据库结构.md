...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.353`。当前运行库为 SQLite；暂停的租户化草案不代表当前实现或当前产品合同。`v0.1.349` 冻结 Product 和旧生产写入，`v0.1.350` 统一新 Material 工单状态机，`v0.1.351-v0.1.352` 交付操作手册与可留档恢复演练。`v0.1.353` 进入 Product 外键退出的扩展阶段：只增加可空 Material 投影和索引，暂不增加物理外键、收紧非空或删除旧列。Product 外键退出表、迁移顺序和状态规则见[单厂 MES 产品边界与核心模型收敛](./单厂MES产品边界与核心模型收敛.md)，生产操作见[Product 到 Material 映射与回填](../operations/Product到Material映射与回填.md)。
