...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.357`。当前运行库为 SQLite；暂停的租户化草案不代表当前实现或当前产品合同。`v0.1.354-v0.1.356` 依次建立生产质量批次、来料到生产谱系和客户发货/退货回流，`v0.1.357` 增加部分判定、检验轮次、返工、报废、让步及解冻处置。操作基线见[全流程作业指导书](../operations/user-guide/MES-lite全流程作业指导书-v0.1.357.md)。
...
当前停止线是“供应商批号/来料内部批次 → 流程转移 → 生产投入 FIFO 分配 → 产出内部批次 → 多轮质检与不合格处置 → 发货批次/客户 → 原发货退货 → 独立退货待检批次 → 最终质量去向”。它是批次级正反向追溯，不是逐件序列号；检验项目/附件、跨批次搜索全景和客户指定批次拣货仍待后续切片。
