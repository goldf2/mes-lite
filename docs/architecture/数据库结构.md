...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.348`。当前运行库为 SQLite，模型设计通过 Prisma 保持向 PostgreSQL 迁移的可能性；`docs/minierp/data-model.md` 中的租户字段属于目标草案，不代表当前已实现。`v0.1.346` 为 `Operator` 增加密码失败锁定字段，并新增 `AuthenticationThrottle` 保存登录、注册和管理员安装的来源限流窗口；迁移只增加列和表，不改写既有账号、角色、密码或会话。`v0.1.347` 将 SQLite 运行文件退出 Git，并增加可校验快照与恢复基线；`v0.1.348` 仅校正当前 Coolify 生产域名配置与文档，不改变 Schema。
