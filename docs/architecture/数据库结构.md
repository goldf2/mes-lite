...
本文以 `prisma/schema.prisma` 为唯一实现来源，事实基线为 `v0.1.354`。当前运行库为 SQLite；暂停的租户化草案不代表当前实现或当前产品合同。`v0.1.349` 冻结 Product 和旧生产写入，`v0.1.350` 统一新 Material 工单状态机，`v0.1.351-v0.1.352` 交付操作手册与可留档恢复演练。`v0.1.353` 进入 Product 外键退出的扩展阶段；`v0.1.354` 新增生产产出内部批次、质量检验及待检/可用/冻结库存状态的第一条闭环。Product 外键退出表、迁移顺序和状态规则见[单厂 MES 产品边界与核心模型收敛](./单厂MES产品边界与核心模型收敛.md)，生产操作见[Product 到 Material 映射与回填](../operations/Product到Material映射与回填.md)，质量操作见[生产产出批次质检与库存状态](../operations/生产产出批次质检与库存状态.md)。
...
当前停止线是“生产产出 → 内部批次 → 待检 → 整批放行/冻结”。来料接收尚未生成统一内部批次，生产投入和发货尚未分配批次，因此不能据此承诺供应商到客户的完整正反向谱系。部分放行、复检、让步、返工、报废和解冻也尚未实现。
