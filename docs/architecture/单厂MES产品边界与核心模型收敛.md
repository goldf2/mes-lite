...
1. 在生产数据库运行 `npm run audit:model-convergence`，保存只读盘点结果。
2. 人工建立所有未映射 `Product → Material` 的确认表，尤其核对非零库存、流水、成本和单据引用。
3. 先增加可空 `materialId` 关系，分批回填并逐表核对数量、金额、引用数和孤儿记录。
4. 仅在必要的短窗口内双写新旧关系；结构守卫禁止新增依赖范围。
5. 当每张表的旧引用数为零后，依次收紧 `materialId`、移除兼容读写，最后删除 `Product` 外键。
...
该命令只读统计 Product 映射、旧外键引用、缺失 Material 投影、Product 独占库存、旧生产记录和迁移阻断项，不修改数据库。

`v0.1.349` 开发机本地数据库快照显示：2 个 Product 均未按编码映射 Material；2 张生产订单和 1 张发货单缺少 `materialId`；存在 2 条 Product 独占库存但数量/流水风险统计为 0；另有 4 条旧领料记录。因此 `readyForProductForeignKeyMigration=false`。这只是本地开发库证据，不代表生产环境；生产结论必须在备份完成后重新审计。
