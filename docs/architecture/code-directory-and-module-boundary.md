...
| `app/HomeApp.tsx` | 1045 行 | 已提取账号菜单、导航图标、页面宿主、页面连续性和工作区偏好控制器；继续只承担应用壳装配 |
| `app/components/shell/` | 7 个文件 | 已建立账号菜单、导航图标、页面宿主、渲染适配器和两类状态控制器的公共应用壳边界 |
...
- `HomeApp.tsx` 不再直接读写页面连续性存储，不再直接请求工作区偏好接口，行数从 1233 行降至 1045 行。
- `verify:shell-controllers` 阻止上述职责重新回流应用壳，并把 `HomeApp.tsx` 的当前规模上限固定为 1100 行。

下一步可以继续拆分桌面导航展开、自动隐藏、尺寸调整和菜单排序等纯交互状态；页面注册与菜单分类仍只使用现有单一事实源。
