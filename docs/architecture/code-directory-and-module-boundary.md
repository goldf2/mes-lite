...
- `verify:structural-goal` 固定最终结构底线：114 条 Route Handler 直连 Prisma 为 0、UI 直接请求为 0、超 800 行页面为 0、超 300 行路由为 0，且所有模块必须存在公开入口。
- 38 个注册页面继续强制公共顶部工具栏，并由 6 类公共骨架承载；本轮不修改页面视觉和交互，页面问题留到结构目标完成后的独立修复阶段。
