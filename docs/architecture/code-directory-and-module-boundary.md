...
| `modules/` | 567 个 TypeScript/TSX 文件 | 当前有 17 个领域与平台模块；库存模块承载仓库数字孪生白板和独立盘点校准，并将批次收货、FIFO 消耗/恢复、发货欠库补账、生产投入分配、发货/退货批次、统一收发过账、库位转移和库存流水日期范围统计收敛到独立内部服务；外部领域通过库存模块公开出口调用。生产模块提供可被流程转移页与仓库白板共同复用的转移录入/快捷确认组件并承载 BOM 快捷生产日报，销售模块独立承载发货状态与批次追溯组件，文档模块承载批量导入、分类字段和同类别批量修改，附件模块统一承载表格直览与 CAD 派生预览，运维工具模块承载数据故障明细导出，系统设置模块管理多引擎切换 |
| `app/api/` | 165 个 `route.ts` | 全部为薄 HTTP 适配层；仓库全景和库存流水统计使用只读查询适配器，直接访问 Prisma 的路由仍为零 |
...
- `modules/business-documents` 作为跨销售、来料和生产领域的单据展示平台：`contracts` 集中 7 类单据定义，通用 `server` 负责来料、销售订单、退货、流程转移、生产订单和派工的查询投影、A4 PDF 渲染、归档缓存和强制重生成；发货业务专用的 `modules/sales/server/shipment-document-service.ts` 统一承载客户版与内部留档版，通用入口只负责转发，避免与旧发货投影并行生成不同文件。
...
- 打印 Route Handler 只保留单据类型、受众/处置参数、资源权限、生成权限、领域服务调用和 PDF HTTP 响应；发货请求额外加载有效数据范围并统一执行已发货状态门禁。旧 `lib/business-document-pdf.ts` 已删除。
- `verify:business-document-print` 使用运行后删除的临时完整 SQLite 和上传目录，覆盖通用业务单据归档、客户/内部发货双受众、预览/下载字节复用、源版本变化留存历史和旧发货渲染器清零；不会触碰本机测试库、上传目录或服务器数据。
- Office/CAD 附件派生 PDF、原始附件下载和 SOP/用户手册静态 PDF 不属于运行时业务单据输出，继续由附件/文档各自管线负责；系统生成的业务 PDF 不混入原始附件列表。
- 跨领域服务端能力通过模块根目录的 `server-index.ts` 公开出口提供；客户端仍只能使用 `index.ts`，不得把 PDF 引擎、文件系统或 Prisma 依赖带入浏览器。
...
## 79. v0.1.459 统一物料编码与兼容身份归属

- `modules/materials/server/material-product-identity.ts` 集中 Product/Material 身份解析与编码投影，跨领域经既有 `lib/material-product.ts` 兼容桥复用，不新增各领域直连物料服务端的深层导入。
- `Material.code` 是编码事实源；兼容产品保留原 ID，显式绑定 `materialId` 并使用同码 SKU。显式绑定优先，未绑定的旧编码只作为无歧义候选；真实 `MAT-A` 与 `A` 不自动等价。
- `modules/operations-tools/server/material-code-normalization-service.ts` 继续拥有“预览 → 执行”事务与冲突检查；产品占码或多物料匹配必须阻断整批，部署不自动改写存量。身份与同码修复不改 BOM 版本/数量、库存成本和历史快照。
- 工艺页面继续复用 `ProductionEngineeringPageShell`、`MaterialChoiceSearch` 和公共搜索排序；列表与编辑使用显式物料身份，扫码和发货 PDF 保留完整真实编码。未新增页面、API、权限或 Prisma 模型。
