...
首次扫描基线：`origin/main`，提交 `5c78233`。当前增量复核基线：提交 `489e31e`，并包含应用壳第一阶段拆分。

| 现状 | 扫描结果 | 判断 |
| --- | ---: | --- |
| `app/page.tsx` | 5 行 | 已收敛为应用入口；应用壳继续由 `HomeApp.tsx` 承担 |
| `app/HomeApp.tsx` | 1233 行 | 已提取账号菜单、导航图标和页面宿主，并移除手工基础菜单；仍需继续拆分导航状态 |
| `app/components/shell/` | 5 个文件 | 已建立账号菜单、导航图标、页面宿主和渲染适配器的公共应用壳边界 |
...
下一步继续拆分 `HomeApp.tsx` 的导航控制、工作区偏好和 URL 连续性状态，不回退为第二套菜单或页面注册数据源。
