...
| `modules/materials/ui/MaterialPage.tsx` | 2432 行 | 数据契约、HTTP client、详情、编辑表单和批量导入均已拆出；列表/卡片和 BOM 编辑状态仍需继续分离 |
...
| `modules/` | 31 个文件 | 已有工作台、生产、库存、配置、物料和 BOM 模块；物料/BOM 已形成 `contracts/client/model/ui` 分层 |
...
下一步应在既有 `materials` 和 `bom` 模块内继续拆分列表/卡片、BOM 草稿状态和全景子视图；页面注册与菜单分类仍只使用现有单一事实源。
