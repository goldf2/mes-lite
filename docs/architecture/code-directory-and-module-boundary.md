...
扫描基线：`origin/main`，提交 `5c78233`。

| 现状 | 扫描结果 | 判断 |
| --- | ---: | --- |
| `app/page.tsx` | 3319 行 | 同时承担应用壳、导航、页面选择、部分业务请求与仪表盘，属于首要冲突热点 |
| `app/components/` 根目录 | 61 个文件 | 公共原语、领域页面、业务弹窗和系统页面混放，文件归属不清 |
| `MaterialPage.tsx` | 3258 行 | 物料、BOM、附件和多种视图交织，应迁入物料领域并继续拆分 |
| `SystemPage.tsx` | 2975 行 | 业务配置、系统设置和维护工具混放，应按业务配置、系统设置、运维工具拆分 |
| `MaterialInPage.tsx` | 1624 行 | 来料页面和录入流程高度集中，应迁入来料领域 |
| `WorkInstructionPage.tsx` | 1594 行 | 文档资源、编辑器、附件和关联编辑集中，应迁入文档领域 |
| `MaterialPanoramaPage.tsx` | 1485 行 | 属于物料领域的专用展示，不应继续留在公共组件根目录 |
| `prisma/schema.prisma` | 1456 行 | 当前继续作为单一事实源，不为目录整齐强拆 Schema |
| `lib/` | 48 个根文件 | 领域规则、平台基础设施、格式化工具和配置混放 |
| `app/api/` | 已按 URL 资源分组 | 路径结构基本合理，但部分路由仍直接承载大量领域规则 |
