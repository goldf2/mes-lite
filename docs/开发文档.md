...
>
> 事实基线：`v0.1.355` / 2026-08-13。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
| 工作区 | 当前主要能力 | 暂不包含 |
| --- | --- | --- |
| MES | 物料、BOM、产品文档、设备、生产订单、派工、流程转移、来料/产出内部批次、生产投入 FIFO 分配和父子谱系、库存状态及整批质检放行/冻结 | APS 高级排程、设备协议全覆盖、发货/退货到客户的完整批次谱系、部分放行和完整不合格处置 |
...
当前 `prisma/schema.prisma` 包含 **71 个数据模型**，建议按数据不变式而不是表名孤立理解：
...
当前有 124 个 `app/api/**/route.ts` 文件，覆盖：
...
1. **局部契约脚本**：`package.json` 当前有 82 个 `verify:*` 命令，覆盖结构目标、模型收敛和 Product→Material 映射回填、权限、身份认证、可信操作人、模块边界、生产工程、生产订单、生产质量批次、来料到生产批次谱系、旧执行兼容与派工、物料、销售、文档、来料、库存余额与库存流水、系统设置、导航、附件及 AI 文档识别、BOM 结构、版本生命周期与成本、业务配置、运行时备份恢复/readiness、运维维护工具、工作台、扫码打印、业务流程和开发文档事实基线。
...
- `v0.1.355` 已将供应商批号、来料内部批次、流程转移批次余额、生产投入 FIFO 分配、投入产出谱系、待检与整批放行/冻结连接起来；仍缺发货/退货到客户的批次边、部分放行、返工/报废/让步处置。
