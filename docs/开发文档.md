...
>
> 事实基线：`v0.1.284` / 2026-08-09。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
当前已完成统一页面注册、权限感知的应用导航控制器、页面连续性/偏好/桌面导航控制器，以及工作台、生产、库存、业务配置、物料、BOM、系统设置和运维工具八个领域模块。`HomeApp.tsx` 已从 1233 行降至 522 行；`MaterialPage.tsx` 已从 2432 行降至 887 行。BOM 草稿状态进入 `modules/bom`；业务资料与规则进入 `modules/configuration`；显示、导航和 AI 设置进入 `modules/system-settings`；数据工具、归档和审计进入 `modules/operations-tools`；加工工艺、物料路线、工程成本、搜索模型与服务端事务规则进入 `modules/production`；物料请求契约、组合查询、图片装配和写入事务进入 `modules/materials`。`SystemPage.tsx` 因此从 1,949 行降至 31 行，只作为旧渲染键的领域分派兼容层；生产工程和物料 API 也已降为权限、校验、审计和 HTTP 响应适配。自动边界守卫阻止这些职责回流。但少数历史领域页仍然过大，因此应定位为“公共骨架已稳定、领域代码正在渐进迁移”，而不是已完成全领域模块化。
...
1. **局部契约脚本**：`package.json` 当前有 57 个 `verify:*` 命令，覆盖权限、骨架、模块边界、生产工程前后端、物料前后端、系统设置、导航、库存、附件、BOM、单位、业务流程和开发文档事实基线。
