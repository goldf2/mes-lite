...
>
> 事实基线：`v0.1.386` / 2026-08-15。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
- `v0.1.382` 已在此前闭环和轻量发布树基线上，将 MES、MRP、ERP 的工作区切换收敛为统一 MES 工作台；`v0.1.383` 增加一个公开 OSS 基地址对应精确版本 PDF/DOCX 的帮助中心下载入口和带 SHA-256 的发布清单；`v0.1.384` 增加默认只读、备份后显式应用的站内文档库登记维护包；`v0.1.385` 让 GitHub Actions 失败 annotation 直接指向具体基线子检查；`v0.1.386` 修复单实例 SQLite 在多个岗位同时登录时的认证写竞争。全部页面、权限、接口和业务数据保留，历史导航配置自动合并。系统仍缺逐件序列号、SPC/过程能力、复杂审批、部门/班组范围、自动设备采集、停机原因、节拍和 OEE。
