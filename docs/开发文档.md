...
>
> 事实基线：`v0.1.369` / 2026-08-14。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
| 设备 | `equipment` | 设备台账、工作中心归属、开停机/故障/恢复事件时间线 |
...
当前 `prisma/schema.prisma` 包含 **78 个数据模型**，建议按数据不变式而不是表名孤立理解：
...
| 设备运行 | `WorkCenter`, `Equipment`, `EquipmentEvent` | 设备归属、当前状态快照及开停机/故障/恢复事件时间线 |
...
- 当前运行时有 49 个权限资源，通用操作为“查、增、改、归档、授权”；质量使用查看、判定、处置和授权放行四个独立资源，生产另拆发布、实绩登记、实绩确认和实绩冲销四个命令资源，设备运行命令使用独立 `equipmentEvents.update`，流程转移、BOM 结构、主数据、系统设置和运维工具也已分别授权。
...
当前有 132 个 `app/api/**/route.ts` 文件，覆盖：

- 认证、操作人员、员工、权限组和审计。
- 物料、BOM、作业文档、附件、客户和供应商。
- 生产订单、班后实绩、派工、生产日报和流程转移。
- 库存、来料、销售订单、发货、退货和成本。
- 设备基础资料、工作中心和设备运行事件时间线。
...
1. **局部契约脚本**：`package.json` 当前有 98 个 `verify:*` 命令，覆盖结构目标、模型收敛和 Product→Material 映射回填、岗位任务与命令权限、细粒度资源、设备事件状态机/HTTP 越权、人员数据范围、归档三重授权、身份认证、可信操作人、模块边界、生产工程、质量、批次、销售、文档、库存、系统设置、附件、BOM、恢复演练、SOP、CI 编排和开发文档事实基线。GitHub Actions 先生成生产构建并执行应用级恢复演练，再运行不会改写 `.next` 的 23 项领域/治理基线。
...
- `v0.1.369` 已在原 48 个细分资源上增加独立设备运行事件资源，并交付开机、停机、故障、恢复及持续时间回放；仍缺逐件序列号、检验项目/附件、复杂审批流、部门/班组范围、设备点检/保养/维修工单和 OEE。
