...
>
> 事实基线：`v0.1.360` / 2026-08-13。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
| 工作区 | 当前主要能力 | 暂不包含 |
| --- | --- | --- |
| MES | 物料、BOM、产品文档、设备、生产订单、派工、流程转移、来料/产出/退货内部批次、生产投入与发货 FIFO 分配、客户去向和退货回流、库存状态及整批质检放行/冻结 | APS 高级排程、设备协议全覆盖、序列号、部分放行和完整不合格处置 |
...
统一页面注册表共有 **41 个页面模块定义**：39 个稳定业务入口（含质量任务与批次追溯），以及生产订单的 `create`/`detail` 内部形态。
...
当前 `prisma/schema.prisma` 包含 **74 个数据模型**，建议按数据不变式而不是表名孤立理解：
...
- 当前运行时有 48 个权限资源，通用操作为“查、增、改、归档、授权”；质量使用查看、判定、处置和授权放行四个独立资源，生产另拆发布、实绩登记、实绩确认和实绩冲销四个命令资源，流程转移、BOM 结构、主数据、系统设置和运维工具也已分别授权。
...
当前有 129 个 `app/api/**/route.ts` 文件，覆盖：
...
当前还没有自动生成的 OpenAPI 规范。对外集成的新 API 应建立契约文档，而不把 124 个路由手工复制到本手册。
...
1. **局部契约脚本**：`package.json` 当前有 89 个 `verify:*` 命令，覆盖结构目标、模型收敛和 Product→Material 映射回填、岗位任务与命令权限、细粒度资源及真实 HTTP 403、身份认证、可信操作人、模块边界、生产工程、生产订单、生产质量批次与不合格处置、来料到生产批次谱系、客户发货退货批次追溯、跨批次搜索全景、旧执行兼容与派工、物料、销售、文档、来料、库存余额与库存流水、系统设置、导航、附件及 AI 文档识别、BOM 结构、版本生命周期与成本、业务配置、运行时备份恢复/readiness、运维维护工具、工作台、扫码打印、业务流程和开发文档事实基线。
...
- `v0.1.360` 已在权限驱动任务区和四类生产命令基础上，将宽泛资源拆为 48 个业务、配置与运维资源，并交付七个新增岗位预置；仍缺逐件序列号、检验项目/附件、复杂审批流、岗位数据范围和个人临时授权元数据。
- MRP-lite 没有完整的净需求、短缺展开和采购/生产建议。
- ERP-lite 不包含财务、税务、应收应付和完整采购。
- 权限已有质量四层和生产四类命令资源，但其他高风险动作、完整原子功能目录及本人/工作中心/库位数据范围仍未完成。
