...
>
> 事实基线：`v0.1.303` / 2026-08-09。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
当前已完成统一页面注册、权限感知的应用导航控制器、页面连续性/偏好/桌面导航控制器，并建立工作台、生产、库存、业务配置、物料、BOM、系统设置、运维工具、文档、来料、身份权限和销售 12 个领域模块。`HomeApp.tsx` 已从 1233 行降至 522 行；`MaterialPage.tsx` 已从 2432 行降至 709 行。BOM 草稿、结构规则、查询装配、方案保存事务、成本输入契约、成本纯计算、成本工作区查询和快照写入进入 `modules/bom`；未注册但保留的 BOM 成本页也从根目录迁入模块，并从 720 行降至 490 行且不再直接访问接口。业务资料与规则进入 `modules/configuration`；显示、导航和 AI 设置进入 `modules/system-settings`；数据工具、归档和审计进入 `modules/operations-tools`；加工工艺、物料路线、工程成本、搜索模型、服务端事务规则，以及派工和流程转移的页面、契约、HTTP 客户端进入 `modules/production`，两个执行页面均不再直接访问接口；物料请求契约、组合查询、图片装配、全景聚合、CSV 导入解析/事务和写入事务进入 `modules/materials`；作业文档契约、请求 client、正文与附件查询、关联校验和写入事务进入 `modules/documents`；库存查询、包装穿透、余额一致性和调整事务进入 `modules/inventory`；人员和权限页面、HTTP client、输入契约、组编码规则、资源级授权判断与权限组/人员组原子写入进入 `modules/identity-access`；销售订单、发货和退货页面，以及对应 HTTP client、展示契约和视图规则统一进入 `modules/sales`。`SystemPage.tsx` 从 1,949 行降至 31 行；权限 API 从 350 行降至 79 行，人员 API 降至 41 行，BOM 成本 API 从 340 行降至 45 行。自动边界守卫阻止这些职责回流。当前已无超过 800 行的页面和超过 300 行的 API，但仍有 5 个存量根级页面，其中 `SystemPage` 是刻意保留的兼容分派层，因此真正待迁移领域页为 4 个。
...
1. **局部契约脚本**：`package.json` 当前有 64 个 `verify:*` 命令，覆盖权限、身份权限模块、骨架、模块边界、生产工程与派工、物料、销售、文档、来料和库存前后端、系统设置、导航、附件、BOM 结构与成本、单位、业务流程和开发文档事实基线。
