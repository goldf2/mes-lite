...
>
> 事实基线：`v0.1.395` / 2026-08-16。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
- `v0.1.382` 已在此前闭环和轻量发布树基线上，将 MES、MRP、ERP 的工作区切换收敛为统一 MES 工作台；`v0.1.383-v0.1.384` 建立单 OSS 地址的 SOP 下载和受控站内登记；`v0.1.385-v0.1.387` 完善 CI 可观测性、SQLite 登录写竞争和生产审计历史基线；`v0.1.388-v0.1.393` 完成主模块首页入口、视频帮助和桌面导航交互；`v0.1.394` 以当前代码事实统一系统上下文、部署、模块、业务流程、关键时序和 17 模块完备性审查；`v0.1.395` 建立 7 个跨页面常用工作流视频 SOP 分镜、批量生成与自动输出检查。全部业务页面、权限资源、接口和数据模型保留。系统仍缺逐件序列号、SPC/过程能力、复杂审批、部门/班组范围、自动设备采集、停机原因、节拍和 OEE。
