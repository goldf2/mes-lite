...
>
> 事实基线：`v0.1.275` / 2026-08-09。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
当前已完成统一页面注册、权限感知的应用导航控制器、页面连续性/偏好/桌面导航控制器，以及工作台、生产、库存、业务配置、物料和 BOM 六个领域模块。`HomeApp.tsx` 已从 1233 行降至 522 行，不再直接组装菜单数据；物料/BOM 的契约、HTTP client 和 UI 已分层，物料编辑与批量导入也已成为各自拥有状态和动作的领域切片。但少数历史领域页仍然过大，因此应定位为“公共骨架已稳定、领域代码正在渐进迁移”，而不是已完成全领域模块化。
