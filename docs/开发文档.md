...
>
> 事实基线：`v0.1.366` / 2026-08-13。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
1. **局部契约脚本**：`package.json` 当前有 96 个 `verify:*` 命令，覆盖结构目标、模型收敛和 Product→Material 映射回填、岗位任务与命令权限、细粒度资源、人员数据范围及真实 HTTP 403、身份认证、可信操作人、模块边界、生产工程、生产订单、生产质量批次与不合格处置、来料到生产批次谱系、客户发货退货批次追溯、跨批次搜索全景、旧执行兼容与派工、物料、销售、文档、来料、库存余额与库存流水、系统设置、导航、附件及 AI 文档识别、附件路径可移植性、BOM 结构、版本生命周期与成本、业务配置、生产数据库审计、运行时备份恢复/readiness、应用级恢复演练、运维维护工具、工作台、扫码打印、业务流程、SOP 覆盖与影响声明、CI 编排和开发文档事实基线。GitHub Actions 先生成生产构建并执行应用级恢复演练，再运行不会改写 `.next` 的 19 项领域/治理基线。
