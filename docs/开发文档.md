...
>
> 事实基线：`v0.1.314` / 2026-08-10。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
`v0.1.314` 新增横切 `modules/attachments`，因此当前领域/平台模块目录总数为 14；上段列举的 13 个业务领域模块不含该通用附件平台模块。附件页面调用、领域规则和服务端生命周期已形成 client/contracts/domain/server 分层。
