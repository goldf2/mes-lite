...
>
> 事实基线：`v0.1.359` / 2026-08-13。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
- 当前运行时有 30 个权限资源，通用操作为“查、增、改、归档、授权”；质量使用查看、判定、处置和授权放行四个独立资源，生产另拆发布、实绩登记、实绩确认和实绩冲销四个命令资源。
...
1. **局部契约脚本**：`package.json` 当前有 87 个 `verify:*` 命令，覆盖结构目标、模型收敛和 Product→Material 映射回填、岗位任务与命令权限、身份认证、可信操作人、模块边界、生产工程、生产订单、生产质量批次与不合格处置、来料到生产批次谱系、客户发货退货批次追溯、跨批次搜索全景、旧执行兼容与派工、物料、销售、文档、来料、库存余额与库存流水、系统设置、导航、附件及 AI 文档识别、BOM 结构、版本生命周期与成本、业务配置、运行时备份恢复/readiness、运维维护工具、工作台、扫码打印、业务流程和开发文档事实基线。
...
- `v0.1.359` 已在既有可搜索批次闭环上增加权限驱动的生产、质量、仓储和系统任务区，并拆分四类生产高风险命令；仍缺逐件序列号、检验项目/附件、复杂审批流和岗位数据范围。
