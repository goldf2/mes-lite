...
>
> 事实基线：`v0.1.295` / 2026-08-09。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
当前已完成统一页面注册、权限感知的应用导航控制器、页面连续性/偏好/桌面导航控制器，以及工作台、生产、库存、业务配置、物料、BOM、系统设置、运维工具和文档九个领域模块。`HomeApp.tsx` 已从 1233 行降至 522 行；`MaterialPage.tsx` 已从 2432 行降至 709 行。BOM 草稿、结构规则、查询装配和方案保存事务进入 `modules/bom`，BOM 列表/编辑工作区与双形态工具栏进入独立任务组件；业务资料与规则进入 `modules/configuration`；显示、导航和 AI 设置进入 `modules/system-settings`；数据工具、归档和审计进入 `modules/operations-tools`；加工工艺、物料路线、工程成本、搜索模型与服务端事务规则进入 `modules/production`；物料请求契约、组合查询、图片装配、全景聚合、CSV 导入解析/事务和写入事务进入 `modules/materials`；作业文档契约、请求 client、正文与附件查询、关联校验和写入事务进入 `modules/documents`；库存查询、包装穿透、余额一致性和调整事务进入 `modules/inventory`。`SystemPage.tsx` 从 1,949 行降至 31 行；生产工程、物料、物料全景、物料导入、BOM、作业文档和库存 API 均已降为 HTTP 适配。自动边界守卫阻止这些职责回流。当前已无超过 800 行的页面，超过 300 行的存量 API 从 6 个降至 2 个，但仍有存量根级领域页和巨型 API 需要渐进迁移，因此不能视为全领域模块化已经完成。
...
1. **局部契约脚本**：`package.json` 当前有 60 个 `verify:*` 命令，覆盖权限、骨架、模块边界、生产工程、物料、文档、来料和库存前后端、系统设置、导航、附件、BOM、单位、业务流程和开发文档事实基线。
