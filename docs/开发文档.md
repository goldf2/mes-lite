...
>
> 事实基线：`v0.1.358` / 2026-08-13。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
1. **局部契约脚本**：`package.json` 当前有 85 个 `verify:*` 命令，覆盖结构目标、模型收敛和 Product→Material 映射回填、权限、身份认证、可信操作人、模块边界、生产工程、生产订单、生产质量批次与不合格处置、来料到生产批次谱系、客户发货退货批次追溯、跨批次搜索全景、旧执行兼容与派工、物料、销售、文档、来料、库存余额与库存流水、系统设置、导航、附件及 AI 文档识别、BOM 结构、版本生命周期与成本、业务配置、运行时备份恢复/readiness、运维维护工具、工作台、扫码打印、业务流程和开发文档事实基线。
...
- `v0.1.358` 已将供应商批号、来料内部批次、生产投入与产出、多轮质量判定/处置、客户发货、退货隔离批次和退货质检连接为可搜索的正反向全景；仍缺逐件序列号、检验项目/附件和复杂审批流。
