...
>
> 事实基线：`v0.1.280` / 2026-08-09。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
当前已完成统一页面注册、权限感知的应用导航控制器、页面连续性/偏好/桌面导航控制器，以及工作台、生产、库存、业务配置、物料、BOM 和运维工具七个领域模块。`HomeApp.tsx` 已从 1233 行降至 522 行，不再直接组装菜单数据；`MaterialPage.tsx` 已从 2432 行降至 887 行，物料详情、编辑、批量导入、卡片、表格、分页、页内选项和显示偏好均为独立切片。BOM 草稿状态、投入/产出互斥、单位换算、脏状态和保存用例由 `modules/bom` 统一拥有，物料页只协调入口和刷新。数据工具、归档和审计已从 `SystemPage.tsx` 迁入 `modules/operations-tools`，并由自动边界守卫阻止职责回流。但少数历史领域页仍然过大，因此应定位为“公共骨架已稳定、领域代码正在渐进迁移”，而不是已完成全领域模块化。
...
1. **局部契约脚本**：`package.json` 当前有 53 个 `verify:*` 命令，覆盖权限、骨架、模块边界、导航、库存、附件、BOM、单位、业务流程和开发文档事实基线。
