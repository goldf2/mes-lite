...
>
> 事实基线：`v0.1.433` / 2026-08-22。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
统一页面注册表共有 **46 个页面模块定义**：44 个稳定入口（含帮助中心、BOM 快捷生产日报及库存盘点校准、质量任务、设备点检、设备维保、CAD 文件预览设置与批次追溯），以及生产订单的 `create`/`detail` 内部形态。
