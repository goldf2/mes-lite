...
>
> 事实基线：`v0.1.455` / 2026-09-04。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
产品只提供一个统一 MES 工作台，不再要求用户切换 MES、MRP、ERP 三个顶层工作区。左上角保留可配置名称和显隐的模块按钮：当前主按钮默认显示为 MES-lite，MRP、ERP 是不可点击的未来能力预留；系统版本移到左侧导航底部。MRP-lite 和 ERP-lite 是同一制造业务链中的计划与经营辅助能力域；页面、权限、数据和审计继续共用同一套事实源，并按角色过滤功能入口。
...
“系统设置 → 导航设置”可修改左上角模块按钮名称与 MRP/ERP 预留按钮显隐，并调整统一 MES 工作台的一级功能区顺序、页面显示名称和页面顺序。按钮配置不改变页面 ID、权限、数据或领域模块；MES-lite 主按钮始终显示，预留按钮不可点击。历史三工作区配置在读取和保存时自动合并；MRP-lite/ERP-lite 的责任边界继续用于产品规划和接口所有权，不再作为工作区切换。

## 4. 当前页面与功能地图

统一页面注册表共有 **48 个页面模块定义**：46 个稳定入口（含帮助中心、仓库数字孪生白板、BOM 快捷生产日报、独立库存盘点、质量任务、设备点检、设备维保、CAD 文件预览设置与批次追溯），以及生产订单的 `create`/`detail` 内部形态。
...
| 生产 | `orders`, `dailyInventory`, `dispatch`, `flowTransfers`, `create`, `detail` | 生产订单、BOM 快捷生产日报、派工、流程转移、订单新增/详情 |
...
| 库存 | `stocks`, `inventoryCount`, `stockMovements`, `lotPanorama` | 总库存、独立库存盘点、库存流水与批次追溯 |
...
| 系统设置 | `displaySettings`, `navigationSettings`, `cadPreviewSettings`, `aiSettings` | 显示效果、统一导航、CAD 文件预览引擎和 AI 服务 |
...
- 库存调整必须指定库位、填原因并写流水。
- “生产日报”默认是生产领域的 BOM 快捷转换：省略生产订单和派工，原子记录实际投入与主产出；产出可直接可用，也可进入待检批次并生成质量任务。第二页签是库存领域的每日盘点校准：一张盘点只选一个库位，可加入多种物品并按实盘数整单校准，不创建生产事实或质检记录。两种模式都不能绕过 FIFO 与受控库存保护。
- 成本来自来料成本层、消耗记录和 BOM/生产计算，不在物料表中维护动态“当前成本”。

### 5.5 销售、发货和退货

- 物料可设默认销售价和币种，销售订单创建时存为快照，手工调价要审计。
- 发货单是独立单据，只保存一个客户和一至多条实际发货物料，不保存销售订单或订单明细 ID，也不回写订单数量与状态。
- 客户与物料维度的未发、待发、已发和超发数量均由当前销售需求与实际发货动态汇总，仅作参考，不限制超发。
...
- PDF、图片、Word、Excel、PowerPoint 等常用文件使用自托管预览；XLS/XLSX/ODS 默认通过 Collabora（LibreOffice 核心）直接打开。DWG/DXF 由隔离的内部 CAD 服务按需生成持久化 PDF 派生预览；原文件始终保留，服务未配置时仍可下载。

附件的上传校验、存储命名、URL 装配、封面切换、旋转、归档接替、草稿绑定/清理和 WOPI 只读直览统一归 `modules/attachments`。页面和其他领域只通过附件模块公开出口调用；九条附件及 WOPI Route Handler 只保留对象级权限、HTTP 文件流、审计和错误映射，不再直接访问 Prisma。附件鉴权同时检查全局附件动作、所属业务资源、对象存在性和暂存账号归属；WOPI 回调叠加短期令牌、proof key 和当前账号/数据范围复核，`/uploads/*` 被 Middleware 阻断静态直读。
...
当前 `prisma/schema.prisma` 包含 **102 个数据模型**，建议按数据不变式而不是表名孤立理解：
...
| 生产 | `ProductionOrder`, `ProductionOrderActual` 及其 Input/Output/Employee/Equipment/WorkInstruction 快照、`Dispatch`, `FlowTransfer`；历史只读兼容 `DailyProductionReport` | 新生产事实只写入订单与实绩；实际设备和作业文件版本随实绩冻结；旧日报禁止创建 |
| 库存与成本 | `Stock`, `StockLocationBalance`, `StockLog`, `InventoryLot`/`Balance`/`Transaction`/`Allocation`/`Genealogy`, `InventoryCostLayer`, `CostLayerConsumption`, `CostRecord` | 总量、库位余额、批次分配/谱系、流水和成本层可对账；每条 Stock 必须且只能归属 Material 或待迁移旧 Product |
| 销售与物流 | `SalesOrder`, `SalesOrderItem`, `MaterialReceipt`, `MaterialIn`, `Shipment`, `ReturnOrder` | 来料单头/多物料明细、单据状态、数量、价格快照和明确来源 |
| 文档 | `WorkInstruction`, `DocumentCategory`, `DocumentAttachment`, `WopiViewSession` | 在线正文、分类、多业务对象附件与表格只读直览短期会话 |
...
- 当前运行时有 63 个权限资源，通用操作为“查、增、改、归档、授权”；质量使用任务查看、标准维护、判定、处置和授权放行五个独立资源，生产另拆发布、实绩登记、实绩确认和实绩冲销四个命令资源，来料另拆收货/拒收和红冲两个命令资源，发货、退货和流程转移另拆八个状态命令资源，设备运行命令使用 `equipmentEvents.update`，点检使用 `equipmentInspections`，保养维修使用 `equipmentMaintenance`，CAD 引擎切换使用 `cadPreviewSettings`。
- `workspace.functionKey` 当前是导航页面功能键，不是完整的原子业务功能目录。
- 生产、来料、发货、退货和流程转移的关键状态命令已独立授权；销售订单确认/取消、派工状态、导入和导出仍按各自现有资源与动作控制，后续按真实岗位风险逐步细分。
...
| 编辑/预览 | Tiptap, pdf.js, PDFKit, Poppler, LibreOffice, `@napi-rs/canvas`，可选内部 CAD 转换服务 | 在线文档、PDF/图片生成、Office 与 DWG/DXF 只读预览 |
...
lib/
  ...
  workspace-navigation-*.ts 统一 MES 工作台菜单配置与旧配置兼容
...
当前已完成统一页面注册、权限感知的应用导航控制器、页面连续性/偏好/桌面导航控制器，并建立 17 个领域与平台模块。`HomeApp.tsx` 已从 1233 行降至 522 行；`MaterialPage.tsx` 已从 2432 行降至 709 行。BOM 草稿、结构规则、查询装配、方案保存事务、成本输入契约、成本纯计算、成本工作区查询和快照写入进入 `modules/bom`；未注册但保留的 BOM 成本页也从根目录迁入模块，并从 720 行降至 490 行且不再直接访问接口。企业资料、供应商/客户、库位、单位、业务规则和员工档案进入 `modules/configuration`；文档类别和工作中心仍从业务配置菜单进入，但分别由 `modules/documents`、`modules/equipment` 唯一拥有，配置模块不复制其页面、契约或请求。公共页内选项通过调用方注入配置模块公开的单位目录能力，不形成公共框架反向依赖。显示、导航和 AI 设置进入 `modules/system-settings`；数据工具、归档、审计、扫码打印和锯切成本进入 `modules/operations-tools`。生产订单及生产实绩的 contracts、domain、server、client 和 UI 已集中到 `modules/production`；工作台的统计契约、client、兼容归一化、指标装配和展示面板进入 `modules/workspace`。加工工艺、物料路线、工程成本、派工和流程转移也归属生产领域；物料、文档、库存、生产日报每日盘点、身份权限和销售分别由各自领域模块拥有。`SystemPage.tsx` 从 1,949 行降至 31 行；自动边界守卫强制所有页面通过所属领域 client 访问接口。当前 46 个注册页面全部使用公共顶部工具栏，所有模块 UI 均为零直接 `fetch`，且已无超过 800 行的页面和超过 300 行的 API。可重复量化指标、当前预算及治理顺序见[代码架构量化基线](./architecture/代码架构量化基线.md)。
...
当前有 165 个 `app/api/**/route.ts` 文件，覆盖：
...
- 系统配置、工作区偏好、库存流水日期范围统计、扫码打印、PDF、AI 和运行就绪检查。
...
需要 Node.js 20+ 和 npm；本地不需要单独安装数据库服务。如需完整 Office/PDF 预览，建议用 Docker 或本机安装 LibreOffice/Poppler；DWG/DXF 预览另需配置遵循内部转换契约的 CAD 服务。
...
| `CAD_PREVIEW_SERVICE_URL` | 隔离的 DWG/DXF 转 PDF 服务根地址；仓库含 LibreDWG 试用服务 | 可选；只能使用私有容器网络，不对公网开放 |
| `CAD_PREVIEW_SERVICE_TOKEN` | CAD 转换服务 Bearer 令牌 | 启用 CAD 预览时建议配置，不入库/不提交 |
| `CAD_PREVIEW_TIMEOUT_MS` | 单次 CAD 转换超时 | 5000–600000，默认 120000 |
...
1. **局部契约脚本**：`package.json` 当前有 134 个 `verify:*` 命令，覆盖发布树体积、结构目标、架构量化非回退、模型收敛和 Product→Material 映射回填、库存所有者数据库不变式、生产 Schema 漂移审计/非覆盖收敛候选、统一库存事务账本、仓库数字孪生只读投影、发货受控负库存与后补入库、BOM 快捷生产日报、生产日报整单盘点、写处理器审计、高风险事务审计、岗位任务与命令权限、生产实绩执行上下文、63 项细粒度资源、质量标准/抽样/趋势、按标准启用的来料检验与红冲、设备事件/点检/维保状态机与 HTTP 越权、人员数据范围、归档三重授权、身份认证、可信操作人、AI 多模态输入、模块边界、生产工程、质量、批次、销售、发货多明细与迁移及发货冲销、文档批量导入与分类字段、库存、系统设置、附件、CAD 转换契约与 LibreDWG 镜像边界、WOPI proof 与 HTTP 取件、BOM、货箱与单据扫码、恢复演练、SOP/全屏帮助、单 OSS 离线发布与站内文档库受控登记、CI 编排和开发文档事实基线。GitHub Actions 先构建并冒烟 LibreDWG 转换镜像，再生成 Web 生产构建、执行应用级恢复演练和不会改写 `.next` 的领域/治理基线。
2. **静态校验**：`npx tsc --noEmit`、`npm run lint`。
3. **生产构建**：`npm run build`。
4. **业务数据验收**：库存、成本和状态流转需使用专用脚本或隔离数据库。

开发期 SOP 只运行 `npm run sop:build:source` 与 `npm run verify:sop`；阶段或商业版本冻结后才运行一次 `npm run sop:build` 和 `npm run verify:sop:artifacts`。DOCX、PDF 与离线 Web 保存在本地 `output/`，不进入 Git/Docker 发布树，详见 [SOP 生成与发布策略](./operations/SOP生成与发布策略.md)。
...
- 小修复先记录到 `docs/releases/PENDING.md`；里程碑、数据迁移、权限/安全、部署或重要业务流程变更则新增 `docs/releases/v<版本>.md` 并更新索引。
- 推送前运行 `npm run verify:release-notes`，确认当前版本已记录。
- 不提交 `.env`、数据库、`.next`、运行时上传文件、密钥和无关用户变更。

#### 14.2.1 待整理记录的位置与格式

- 唯一入口是 `docs/releases/PENDING.md`，不在聊天记录、提交信息或临时文件中另建平行清单。
- 每个小版本一行，字段固定为“版本、日期、变更记录、验证证据、待整理文档”；变更记录写用户可理解的结果，验证证据写命令或实测结果，不只写“已修复”。
- 表格格式固定为：

```markdown
| v0.1.405 | 2026-08-18 | 用户可见的变更与影响边界 | 自动验证命令；真实页面或生产检查结果 | 后续需补的发布说明/SOP/架构文档 |
```

#### 14.2.2 正式发布说明的位置与格式

- 独立文件位于 `docs/releases/v<版本>.md`，标题使用“`# v<版本> <变更主题>`”，正文至少包含“`## 更新内容`、`## 影响范围`、`## 验证结果`”；存在数据或部署风险时再加“`## 回滚`”。
- `docs/releases/README.md` 只索引已完成校对的正式发布说明，不为 `PENDING.md` 中的每条小修复重复建立链接。
- 集中整理时，先把相关待整理行迁入正式文档并更新索引，再从 `PENDING.md` 删除已迁移行；未迁移记录不得清理。
- 交互、数据、权限或流程变化仍应同步修改直接受影响的专题规范；“集中整理”只允许延后发布说明、全局文档基线号和未变流程的 SOP 成品，不能用来隐藏实现与当前规范的差异。
...
2. **确定责任边界**：属于生产执行、计划、经营协同、共享平台还是明确不做；责任边界不等于新增顶层系统入口。
...
- `v0.1.382` 已在此前闭环和轻量发布树基线上，将 MES、MRP、ERP 的工作区切换收敛为统一 MES 工作台；`v0.1.383-v0.1.384` 建立单 OSS 地址的 SOP 下载和受控站内登记；`v0.1.385-v0.1.387` 完善 CI 可观测性、SQLite 登录写竞争和生产审计历史基线；`v0.1.388-v0.1.393` 完成主模块首页入口、视频帮助和桌面导航交互；`v0.1.394` 以当前代码事实统一系统上下文、部署、模块、业务流程、关键时序和 17 模块完备性审查；`v0.1.395` 建立 7 个跨页面常用工作流视频 SOP 分镜、批量生成与自动输出检查；`v0.1.396` 将开发冻结、生产上线与全局治理完成分开，并把剩余外部工作收敛为七个生产验收门禁；`v0.1.397` 按用户决定结束原治理任务并移交人工事项；`v0.1.398-v0.1.400` 根据后续明确需求增加无关联人员账号受限删除、管理员账号资料维护、停用恢复和安全密码重置，不新增模型或迁移；`v0.1.401` 将生产构建门禁迁移到 GitHub Actions 候选分支，开发机不再重复完整生产构建；`v0.1.402` 增加 AI 助手语音和需人工确认的图片识别输入；`v0.1.403` 让销售订单明细复用物料模块公开入口查看只读物料详情；`v0.1.404` 修复来料单右侧已加入清单无法独立滚动的问题。系统仍缺逐件序列号、SPC/过程能力、复杂审批、部门/班组范围、自动设备采集、停机原因、节拍和 OEE。
...
3. 在“计划与物料”功能区实现可解释、需人工确认的 MRP-lite 短缺和建议，不重新建立顶层系统切换。
4. 将 ERP-lite 保持为经营协同责任边界，不在 MES 核心未稳定前扩展财务或另建平行主数据。
...
| MES 主体与计划、经营辅助能力边界 | [MES-lite 混合系统功能边界矩阵](./architecture/MES-MRP-ERP功能矩阵.md) |
...
| 系统设置 | 不改变业务数据语义的显示、导航、文件预览引擎、AI 服务和底层配置 |
