...
当前已完成统一页面注册、权限感知的应用导航控制器、页面连续性/偏好/桌面导航控制器，以及四个领域模块的第一阶段拆分。`HomeApp.tsx` 已从 1233 行降至 522 行，不再直接组装菜单数据。但 `app/components` 仍有较多历史专用组件，因此应定位为“公共骨架已稳定、领域代码正在渐进迁移”，而不是已完成全领域模块化。
