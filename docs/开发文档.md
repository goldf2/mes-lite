...
>
> 事实基线：`v0.1.321` / 2026-08-10。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
`v0.1.314` 新增横切 `modules/attachments`，因此当前领域/平台模块目录总数为 14；上段列举的 13 个业务领域模块不含该通用附件平台模块。`v0.1.315` 至 `v0.1.320` 依次将销售履约、来料、派工、流程转移、往来单位和库位生命周期收口到所属领域服务。`v0.1.321` 将设备与工作中心的输入契约、查询、写入、引用保护和归档恢复统一收口到 `modules/equipment`，两条 API 不再直接访问 Prisma；配置模块只保留工作中心菜单分派，不再持有其页面或数据契约。
