...
统一页面注册表共有 **47 个页面模块定义**：45 个稳定入口（含帮助中心、BOM 快捷生产日报、独立库存盘点、质量任务、设备点检、设备维保、CAD 文件预览设置与批次追溯），以及生产订单的 `create`/`detail` 内部形态。
