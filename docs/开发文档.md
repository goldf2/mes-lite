...
>
> 事实基线：`v0.1.370` / 2026-08-14。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
统一页面注册表共有 **43 个页面模块定义**：41 个稳定入口（含帮助中心、质量任务、设备点检与批次追溯），以及生产订单的 `create`/`detail` 内部形态。
...
| 设备 | `equipment`, `equipmentInspections` | 设备台账、工作中心归属、开停机/故障/恢复事件时间线、周期点检与现场附件 |
...
当前 `prisma/schema.prisma` 包含 **82 个数据模型**，建议按数据不变式而不是表名孤立理解：
...
| 设备运行 | `WorkCenter`, `Equipment`, `EquipmentEvent`, `EquipmentInspectionPlan`/`Item`, `EquipmentInspectionRecord`/`Result` | 设备归属、事件派生状态、不可变周期标准、逐项点检事实、异常故障联动与记录附件 |
...
- 当前运行时有 50 个权限资源，通用操作为“查、增、改、归档、授权”；质量使用查看、判定、处置和授权放行四个独立资源，生产另拆发布、实绩登记、实绩确认和实绩冲销四个命令资源，设备运行命令使用 `equipmentEvents.update`，点检计划与执行使用 `equipmentInspections`，其他领域也已分别授权。
...
当前有 135 个 `app/api/**/route.ts` 文件，覆盖：

- 认证、操作人员、员工、权限组和审计。
- 物料、BOM、作业文档、附件、客户和供应商。
- 生产订单、班后实绩、派工、生产日报和流程转移。
- 库存、来料、销售订单、发货、退货和成本。
- 设备基础资料、工作中心、设备运行事件时间线和周期点检。
...
1. **局部契约脚本**：`package.json` 当前有 101 个 `verify:*` 命令，覆盖结构目标、模型收敛和 Product→Material 映射回填、岗位任务与命令权限、50 项细粒度资源、设备事件/点检状态机与 HTTP 越权、人员数据范围、归档三重授权、身份认证、可信操作人、模块边界、生产工程、质量、批次、销售、文档、库存、系统设置、附件、BOM、恢复演练、SOP/全屏帮助、CI 编排和开发文档事实基线。GitHub Actions 先生成生产构建并执行应用级恢复演练，再运行不会改写 `.next` 的 26 项领域/治理基线。
...
- `v0.1.370` 已在设备事件上增加独立点检资源，并交付周期计划、到期任务、完整项目结果、异常故障联动和现场附件；仍缺逐件序列号、通用检验模板、复杂审批流、部门/班组范围、设备保养/维修工单和 OEE。
