...
>
> 事实基线：`v0.1.431` / 2026-08-22。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
统一页面注册表共有 **45 个页面模块定义**：43 个稳定入口（含帮助中心、质量任务、设备点检、设备维保、CAD 文件预览设置与批次追溯），以及生产订单的 `create`/`detail` 内部形态。
...
当前有 160 个 `app/api/**/route.ts` 文件，覆盖：
...
1. **局部契约脚本**：`package.json` 当前有 126 个 `verify:*` 命令，覆盖发布树体积、结构目标、模型收敛和 Product→Material 映射回填、库存所有者数据库不变式、生产 Schema 漂移审计/非覆盖收敛候选、统一库存事务账本、写处理器审计、高风险事务审计、岗位任务与命令权限、生产实绩执行上下文、62 项细粒度资源、质量标准/抽样/趋势、按标准启用的来料检验与红冲、设备事件/点检/维保状态机与 HTTP 越权、人员数据范围、归档三重授权、身份认证、可信操作人、AI 多模态输入、模块边界、生产工程、质量、批次、销售、发货多明细与迁移、文档批量导入与分类字段、库存、系统设置、附件、CAD 转换契约与 LibreDWG 镜像边界、WOPI proof 与 HTTP 取件、BOM、货箱与单据扫码、恢复演练、SOP/全屏帮助、单 OSS 离线发布与站内文档库受控登记、CI 编排和开发文档事实基线。GitHub Actions 先构建并冒烟 LibreDWG 转换镜像，再生成 Web 生产构建、执行应用级恢复演练和不会改写 `.next` 的领域/治理基线。
