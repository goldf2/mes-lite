...
>
> 事实基线：`v0.1.330` / 2026-08-10。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
1. **局部契约脚本**：`package.json` 当前有 73 个 `verify:*` 命令，覆盖权限、身份认证、身份权限模块、骨架、模块边界、生产工程、生产订单、旧执行兼容与派工、物料、销售、文档、来料和库存前后端、系统设置、导航、附件及 AI 文档识别、BOM 结构与成本、跨领域成本服务、业务配置往来单位与库位、运维维护工具、工作台、锯切计算、扫码打印、单位、业务流程和开发文档事实基线。
