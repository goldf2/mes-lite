...
>
> 事实基线：`v0.1.354` / 2026-08-13。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
| 工作区 | 当前主要能力 | 暂不包含 |
| --- | --- | --- |
| MES | 物料、BOM、产品文档、设备、生产订单、派工、流程转移、来料、库存、库位、生产产出内部批次和整批质检放行/冻结 | APS 高级排程、设备协议全覆盖、来料到客户的完整批次谱系、部分放行和完整不合格处置 |
...
- `v0.1.353` 已为 Product 及五张旧依赖表增加可空 Material 投影。编码候选可用于保持扩展阶段业务连续，但只有签字后的[Product 到 Material 映射与回填](./operations/Product到Material映射与回填.md)才能写入 `Product.materialId`。
...
当前 `prisma/schema.prisma` 包含 **69 个数据模型**，建议按数据不变式而不是表名孤立理解：
...
- 当前运行时有 23 个权限资源，通用操作为“查、增、改、归档、授权”；生产质量判定使用独立 `quality.update`。
...
当前还没有自动生成的 OpenAPI 规范。对外集成的新 API 应建立契约文档，而不把 123 个路由手工复制到本手册。
...
1. **局部契约脚本**：`package.json` 当前有 81 个 `verify:*` 命令，覆盖结构目标、模型收敛和 Product→Material 映射回填、权限、身份认证、可信操作人、身份权限模块、骨架、模块边界、生产工程、生产订单、生产质量批次、旧执行兼容与派工、物料、销售、文档、来料、库存余额与库存流水、系统设置、导航、附件及 AI 文档识别、BOM 结构、版本生命周期与成本、跨领域成本服务、业务配置往来单位与库位、运行时备份恢复/readiness、运维维护工具、工作台、锯切计算、扫码打印、单位、业务流程和开发文档事实基线。
...
- `v0.1.354` 已把新生产实绩产出写入内部批次和待检库存，并支持质检员整批合格放行到可用库存、整批不合格冻结；仍缺来料批次接入、投入批次分配、部分放行、返工/报废/让步处置和从供应商到客户的完整谱系。
