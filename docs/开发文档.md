...
>
> 事实基线：`v0.1.323` / 2026-08-10。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
`v0.1.314` 新增横切 `modules/attachments`，因此当前领域/平台模块目录总数为 14；上段列举的 13 个业务领域模块不含该通用附件平台模块。`v0.1.315` 至 `v0.1.320` 依次将销售履约、来料、派工、流程转移、往来单位和库位生命周期收口到所属领域服务。`v0.1.321` 将设备与工作中心的输入契约、查询、写入、引用保护和归档恢复统一收口到 `modules/equipment`。`v0.1.322` 继续把仅为服务器历史数据保留的旧生产日报兼容链路收口到 `modules/production`：输入、编号、BOM 快照、查询、草稿写入、确认过账和冲销均由领域服务拥有，4 条兼容 API 只保留权限、Schema、审计与 HTTP 映射。旧入口没有恢复，新的生产实绩仍只从生产订单详情进入。`v0.1.323` 将单位新增、修改、删除和使用量装配收口到 `modules/configuration`；单位 API 仅保留鉴权、校验、审计和响应映射，预置及已引用单位保护由配置领域规则和服务统一执行。
