...
- 当前运行时有 62 个权限资源，通用操作为“查、增、改、归档、授权”；质量使用任务查看、标准维护、判定、处置和授权放行五个独立资源，生产另拆发布、实绩登记、实绩确认和实绩冲销四个命令资源，来料另拆收货/拒收和红冲两个命令资源，发货、退货和流程转移另拆七个状态命令资源，设备运行命令使用 `equipmentEvents.update`，点检使用 `equipmentInspections`，保养维修使用 `equipmentMaintenance`，CAD 引擎切换使用 `cadPreviewSettings`。
...
当前有 162 个 `app/api/**/route.ts` 文件，覆盖：
...
1. **局部契约脚本**：`package.json` 当前有 128 个 `verify:*` 命令，覆盖发布树体积、结构目标、模型收敛和 Product→Material 映射回填、库存所有者数据库不变式、生产 Schema 漂移审计/非覆盖收敛候选、统一库存事务账本、BOM 快捷生产日报、生产日报整单盘点、写处理器审计、高风险事务审计、岗位任务与命令权限、生产实绩执行上下文、62 项细粒度资源、质量标准/抽样/趋势、按标准启用的来料检验与红冲、设备事件/点检/维保状态机与 HTTP 越权、人员数据范围、归档三重授权、身份认证、可信操作人、AI 多模态输入、模块边界、生产工程、质量、批次、销售、发货多明细与迁移、文档批量导入与分类字段、库存、系统设置、附件、CAD 转换契约与 LibreDWG 镜像边界、WOPI proof 与 HTTP 取件、BOM、货箱与单据扫码、恢复演练、SOP/全屏帮助、单 OSS 离线发布与站内文档库受控登记、CI 编排和开发文档事实基线。GitHub Actions 先构建并冒烟 LibreDWG 转换镜像，再生成 Web 生产构建、执行应用级恢复演练和不会改写 `.next` 的领域/治理基线。
