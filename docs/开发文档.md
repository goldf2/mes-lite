...
>
> 事实基线：`v0.1.320` / 2026-08-10。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
附件的上传校验、存储命名、URL 装配、封面切换、旋转、归档接替以及草稿绑定/清理统一归 `modules/attachments`。页面和其他领域只通过附件模块公开 client 调用；六条附件 Route Handler 只保留权限、HTTP 文件流、审计和错误映射，不再直接访问 Prisma。
...
当前已完成统一页面注册、权限感知的应用导航控制器、页面连续性/偏好/桌面导航控制器，并建立工作台、生产、库存、业务配置、物料、BOM、系统设置、运维工具、文档、来料、身份权限、销售和设备 13 个领域模块。`HomeApp.tsx` 已从 1233 行降至 522 行；`MaterialPage.tsx` 已从 2432 行降至 709 行。BOM 草稿、结构规则、查询装配、方案保存事务、成本输入契约、成本纯计算、成本工作区查询和快照写入进入 `modules/bom`；未注册但保留的 BOM 成本页也从根目录迁入模块，并从 720 行降至 490 行且不再直接访问接口。企业资料、参考资料、业务规则和员工档案进入 `modules/configuration`；供应商/客户、库位、单位、工作中心和文档类别共用参考资料 contracts/client，5 个配置页均不再直接调用 `fetch`；公共页内选项通过调用方注入配置模块公开的单位目录能力，不形成公共框架反向依赖。显示、导航和 AI 设置进入 `modules/system-settings`；数据工具、归档、审计、扫码打印和锯切成本进入 `modules/operations-tools`，运维维护页和两个硬件/成本工具页均通过领域 client 访问接口，归档记录映射和排序成为可测试纯规则，锯切材料、班次和规模计算也保持纯函数。生产订单及生产实绩的 contracts、domain、server、client 和 UI 已集中到 `modules/production`：订单协调页从 535 行降至 448 行，订单创建/详情/候选/确认/取消及实绩工作区/草稿/确认/冲销/删除 API 均为薄层，实绩编号、BOM/成本层快照、投入/多产出、库存成本过账和反向恢复可独立或在临时完整 SQLite 中验证。工作台的统计契约、client、兼容归一化、指标装配和展示面板进入 `modules/workspace`，协调页从 397 行降至 74 行，仪表盘查询、时间窗口、并行聚合和库存预警过滤进入服务，API 从 141 行降至 17 行。加工工艺、物料路线、工程成本、派工和流程转移也归属生产领域；设备台账页面、HTTP client、契约和视图规则进入 `modules/equipment`；物料请求契约、组合查询、图片装配、全景聚合、CSV 导入解析/事务和写入事务进入 `modules/materials`；作业文档契约、请求 client、正文与附件查询、关联校验和写入事务进入 `modules/documents`；库存查询、包装穿透、余额一致性和调整事务进入 `modules/inventory`；人员和权限页面、HTTP client、输入契约、组编码规则、资源级授权判断与权限组/人员组原子写入进入 `modules/identity-access`；销售订单、发货和退货页面，以及对应 HTTP client、展示契约和视图规则统一进入 `modules/sales`。`SystemPage.tsx` 从 1,949 行降至 31 行；权限 API 从 350 行降至 79 行，人员 API 降至 41 行，BOM 成本 API 从 340 行降至 45 行。自动边界守卫现强制所有页面通过所属领域 client 访问接口。当前 38 个注册页面全部使用公共顶部工具栏，所有 `*Page` / `*PageModule` 均为零直接 `fetch`，且已无超过 800 行的页面和超过 300 行的 API。

`v0.1.314` 新增横切 `modules/attachments`，因此当前领域/平台模块目录总数为 14；上段列举的 13 个业务领域模块不含该通用附件平台模块。附件页面调用、领域规则和服务端生命周期已形成 client/contracts/domain/server 分层。`v0.1.315` 进一步让销售订单、发货和退货形成完整的 client/contracts/domain/server 垂直切片，16 条销售履约 API 不再直接访问 Prisma。`v0.1.316` 将来料详情、收货、拒收和整单红冲继续收口到 `modules/receiving`，5 条来料 API 均成为薄层。`v0.1.317` 将派工查询、创建、归档和五态流转收口到 `modules/production`，6 条派工 API 均成为薄层。`v0.1.318` 将流程转移工作区、草稿和确认/冲销事务收口到 `modules/production`，4 条流程转移 API 均成为薄层。`v0.1.319` 将供应商与客户的同构生命周期收敛为配置领域公共服务和 HTTP 适配器，两条 API 只保留资源声明。`v0.1.320` 将库位查询、默认状态和受保护归档事务收口到配置领域，并封闭直接停用绕过归档校验的路径。
...
1. **局部契约脚本**：`package.json` 当前有 69 个 `verify:*` 命令，覆盖权限、身份权限模块、骨架、模块边界、生产工程、生产订单与派工、物料、销售、文档、来料和库存前后端、系统设置、导航、附件、BOM 结构与成本、业务配置往来单位与库位、运维维护工具、工作台、锯切计算、扫码打印、单位、业务流程和开发文档事实基线。
