...
>
> 事实基线：`v0.1.381` / 2026-08-15。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma`、`sop/manifest.json` 和实际 API 为准。
>
...
- `v0.1.381` 已交付设备事件/点检/维保、备件库存/成本/FIFO 批次过账，生产/退货/按标准启用的来料检验、自动抽样、逐项结果、附件、趋势与来料红冲联动，来料/物流状态命令分权，生产实绩实际设备与作业文件版本快照、生产 Schema 漂移非覆盖收敛、写接口审计守卫、统一库存事务账本、数据库冲销约束、库存所有者二选一约束和轻量发布树门禁；仍缺逐件序列号、SPC/过程能力、复杂审批、部门/班组范围、自动设备采集、停机原因、节拍和 OEE。
