...
>
> 事实基线：`v0.1.316` / 2026-08-10。代码会持续变化；当文档与代码冲突时，以 `lib/page-registry.ts`、`lib/permissions.ts`、`prisma/schema.prisma` 和实际 API 为准。
>
...
`v0.1.314` 新增横切 `modules/attachments`，因此当前领域/平台模块目录总数为 14；上段列举的 13 个业务领域模块不含该通用附件平台模块。附件页面调用、领域规则和服务端生命周期已形成 client/contracts/domain/server 分层。`v0.1.315` 进一步让销售订单、发货和退货形成完整的 client/contracts/domain/server 垂直切片，16 条销售履约 API 不再直接访问 Prisma。`v0.1.316` 将来料详情、收货、拒收和整单红冲继续收口到 `modules/receiving`，5 条来料 API 均成为薄层。
